...
#     k-1 dummy columns.
# Effect-size benchmarks (CLAUDE.md §6): continuous 0.10/0.25/0.40 small/med/large;
...
# losf 8 — factor x factor (2x2 cell-means / ANOVA). Both factors 2-level: g1 is
# the bare binary column, g2 contributes g2[2]; interaction g1:g2[2].
ols_ff_a <- list(
  label = "ols_ff_a", losf = 8L,
  # y = 0.50*g1 + 0.40*g2[2] + 0.30*g1:g2[2] + e
  formula = "y ~ g1*g2", family = "ols",
  variable_types = c("g1=(factor, 0.5, 0.5)", "g2=(factor, 0.6, 0.4)"),
  effects = "g1=0.50, g2[2]=0.40, g1:g2[2]=0.30", n = 800L, seed = 2137
)
ols_ff_b <- list(
  label = "ols_ff_b", losf = 8L,
  # y = 0.20*g1 + 0.80*g2[2] + 0.50*g1:g2[2] + e
  formula = "y ~ g1*g2", family = "ols",
  variable_types = c("g1=(factor, 0.5, 0.5)", "g2=(factor, 0.55, 0.45)"),
  effects = "g1=0.20, g2[2]=0.80, g1:g2[2]=0.50", n = 800L, seed = 2138
)
