...
fn linear_factor_with_contrasts_and_tukey_runs() {
    ...
    // without this the contrast entries silently escape the length invariant on the native path.
    assert!(!pr.contrast_pairs.is_empty(), "contrast pairs present");
    let expected_len = pr.target_indices.len() + pr.contrast_pairs.len();
    assert_eq!(
        pr.power_uncorrected.len(),
        expected_len,
        "EP-1: power_uncorrected len == marginals + contrasts"
    );
    assert_eq!(
        pr.power_corrected.len(),
        expected_len,
        "EP-1: power_corrected len == marginals + contrasts"
    );
    assert_eq!(
        pr.ci_uncorrected.len(),
        expected_len,
        "EP-1: ci_uncorrected len == marginals + contrasts"
    );
    assert!(
