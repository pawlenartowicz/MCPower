...
/// τ² = icc / (1 − icc), mirroring the Python port (`model.py:1578`).
/// icc is validated to `[0, 1)` upstream; guard the denominator anyway.
...
/// Latent-scale logistic ICC→τ²: `icc_to_tau_squared(icc) · π²/3`. The random
/// intercept of a logistic GLMM lives on the log-odds scale, whose residual
/// variance is π²/3 (not 1). Mirrors the Python port's binary branch in
/// `_build_cluster_spec_dict` and the R port's `.encode_outcome_and_clusters`.
...
fn assemble_mixed(
    ...
    apply_outcome_options(&mut builder_spec, spec.outcome_options.as_ref(), &spec.var_types)?;
    ...
    let (outcome_kind, estimator, intercept, tau_squared) = match &spec.outcome {
        ...
        MixedOutcome::Binary { baseline_probability } => {
...
/// `non_factor_names.index(name)` (port Task 1.9) and the R port (Task 2.4) —
/// the cross-port column-resolution tripwire test pins all three to agree.
fn resolve_slopes(
    var_types: &[VarType],
    slopes: &[AppSlopeTerm],
...
fn assemble_linear(
    ...
    // Linear-specific fields: override the defaults set by the shared helper.
    builder_spec.report_overall = spec.report_overall;
    builder_spec.contrast_pairs = spec.contrasts.clone();
    builder_spec.test_formula = spec.test_formula.clone();
    builder_spec.scenarios = spec.scenarios.clone();
...
mod tests {
    ...
    fn project_emits_only_nonzero_correlation_pairs() {
        ...
        let spec = project_to_builder_spec(
            &pf(vec!["a", "b", "c"], vec![]),
            &[
                VarType::Numeric { name: "a".into(), distribution: NumericDistribution::Normal, pinned: false },
                VarType::Numeric { name: "b".into(), distribution: NumericDistribution::Normal, pinned: false },
                VarType::Numeric { name: "c".into(), distribution: NumericDistribution::Normal, pinned: false },
            ],
            &[],
            &corr,
            CorrectionMethod::None,
    ...
    fn project_maps_test_selection_to_targets() {
        let mk = |tests: TestSelection| {
            project_to_builder_spec(
                &pf(vec!["x1"], vec![]),
                &[VarType::Numeric { name: "x1".into(), distribution: NumericDistribution::Normal, pinned: false }],
                &[],
                &None,
                CorrectionMethod::None,
                &tests,
    ...
    fn numeric_distribution_maps_to_var_kind() {
        ...
        assert!(matches!(mk(NumericDistribution::RightSkewed), VarKind::RightSkewed));
        assert!(matches!(mk(NumericDistribution::LeftSkewed), VarKind::LeftSkewed));
        assert!(matches!(mk(NumericDistribution::HighKurtosis), VarKind::HighKurtosis));
    ...
    fn neutral_outcome_options_keep_scenario_driven_heteroskedasticity() {
        ...
        .expect("project ok");
        apply_outcome_options(&mut builder_spec, Some(&OutcomeOptions::default()), &var_types)
            .expect("apply ok");
        assert_eq!(builder_spec.heteroskedasticity.driver_var_index, None);
        assert_eq!(builder_spec.residual.distribution, "normal");
        assert!(!builder_spec.residual.pinned, "absent dist must stay unpinned");
    }
    ...
    fn linear_app_spec(scenarios: Vec<engine_spec_builder::input::ScenarioInput>) -> AppSpec {
        AppSpec::Linear(LinearSpec {
            ...
            var_types: vec![VarType::Numeric { name: "x1".into(), distribution: NumericDistribution::Normal, pinned: false }],
    ...
    #[test]
    fn empty_scenarios_assemble_to_one_baseline_contract() {
        let contracts = assemble_spec(&linear_app_spec(vec![])).expect("assemble ok");
        assert_eq!(
            contracts.len(),
            1,
            "empty scenarios → one baseline contract"
        );
    }
    ...
    use crate::app_spec::{AppGroupingRelation, AppGroupingSpec, AppSlopeTerm, ClusterDim, MixedOutcome, MixedSpec};
    ...
    fn mixed_fixture() -> MixedSpec {
        MixedSpec {
            ...
            var_types: vec![
                VarType::Numeric { name: "x1".into(), distribution: NumericDistribution::Normal, pinned: false },
                VarType::Numeric { name: "x2".into(), distribution: NumericDistribution::Normal, pinned: false },
            ],
    ...
    fn assemble_mixed_forwards_cluster_level_vars() {
        ...
        let (contracts, _) = assemble_mixed(&spec)
            .expect("assemble_mixed with cluster_level_vars must not error");
    ...
    fn assemble_mixed_forwards_extra_groupings() {
        ...
        let (contracts, _) = assemble_mixed(&spec)
            .expect("assemble_mixed with extra_groupings must succeed");
        ...
        assert_eq!(cluster.extra_groupings.len(), 1, "crossed grouping must forward");
    }
    ...
    fn assemble_mixed_unknown_slope_predictor_errors() {
        ...
        assert!(
            matches!(assemble_mixed(&spec), Err(AdapterError::UnknownPredictor(_))),
            "unknown slope predictor must error"
        );
    }

    // Cross-port column-resolution tripwire (mirrors Python Task 1.9 / R Task 2.4):
    ...
    fn slope_column_id_is_non_factor_position_not_formula_position() {
        ...
        spec.var_types = vec![
            ...
            VarType::Numeric { name: "x".into(), distribution: NumericDistribution::Normal, pinned: false },
        ];
    ...
    fn assemble_mixed_binary_outcome_sets_binary_glm_and_logit_scale_tau() {
        ...
        let mut spec = mixed_fixture();
        spec.icc = icc;
        spec.outcome = MixedOutcome::Binary { baseline_probability: p };
        ...
        assert!((c.outcome.intercept - expected_intercept).abs() < 1e-10, "intercept = logit(p)");

        let expected_tau = icc / (1.0 - icc) * std::f64::consts::PI.powi(2) / 3.0;
        let cluster = c.generation.cluster.as_ref().expect("cluster present");
        assert!((cluster.tau_squared - expected_tau).abs() < 1e-10, "τ² latent logistic scale");
    }
    ...
    fn binary_mixed_spec_with_scenarios_assembles_to_n_contracts() {
        ...
        let mut spec = mixed_fixture();
        spec.outcome = MixedOutcome::Binary { baseline_probability: 0.3 };
