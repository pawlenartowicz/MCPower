...
/// `ProgressEvent` union in both the WASM worker pool and the Tauri event listener.
/// Any rename or addition of a `kind` value must be mirrored in those consumers.
