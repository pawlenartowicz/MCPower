...
use crate::{
    app_spec::{ClusterDim, EffectSize},
...
/// Single-core twin of `run_find_power` for the WASM worker pool. `n_sims` and
/// `base_seed` are caller-supplied per worker (not read from the spec).
...
/// Single-core twin of `run_find_sample_size` (Grid only) for the WASM worker
/// pool. `n_sims` and `base_seed` are per-worker; `target_power` is read from
/// the spec (run-level, shared).
...
/// intercept is dropped from the return value.
pub fn get_effects_from_data(spec: &AppSpec) -> Result<Vec<EffectSize>, AdapterError> {
