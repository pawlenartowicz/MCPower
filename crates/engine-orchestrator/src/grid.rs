...
pub fn derive_first_achieved(
    ...
    let n_targets = per_n.first().map(|pr| pr.power_corrected.len()).unwrap_or(0);
    (0..n_targets)
...
pub(crate) fn derive_sample_size_outputs(
    ...
    let (first_overall_achieved, fitted_overall) = if has_overall {
        ...
        let overall_counts: Vec<u64> =
            per_n.iter().map(|pr| pr.overall_significant_count).collect();
        let fitted_overall =
...
mod grid_builder_tests {
    ...
    #[test]
    fn fixed_step_unclustered_matches_legacy_walk() {
        let (g, _) = build_grid(30, 50, ByValue::Fixed(5), GridMode::Linear, 1, 1).unwrap();
