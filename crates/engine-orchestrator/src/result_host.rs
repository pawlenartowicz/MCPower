...
//! Python int but an R integer vector; `BoundaryHit` is a 2-D NumPy array but a
//! flat R vector; an `IndexMap` is keyed by int in Python but by string in R).
...
use crate::result::{Ci, CrossingFit, EstimatorExtras, PosthocPower, PowerResult, SampleSizeResult};
use serde::Serialize;
...
pub enum HostValue {
    ...
    /// cols=n_sample_sizes)` order. Python reshapes to a 2-D `uint8` array; R
    /// keeps the flat vector.
    BoundaryHit {
        flat: Vec<u8>,
        rows: usize,
        cols: usize,
...
fn estimator_extras_to_host(fx: &EstimatorExtras) -> HostValue {
    match fx {
        ...
        EstimatorExtras::Glm {
            ...
            (
                "singular_fit_rate",
                HostValue::F64(*singular_fit_rate),
            ),
            (
                "tau_squared_hat_mean",
