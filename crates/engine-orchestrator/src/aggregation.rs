...
/// Wilson score 95% CI for a binomial proportion: with p̂ = k/n,
/// `center = (p̂ + z²/2n) / (1 + z²/n)` and
/// `margin = z·√(p̂(1−p̂)/n + z²/4n²) / (1 + z²/n)`. Mirrors the Python
/// `_wilson_ci` (orchestration.py:63) bit-for-bit at the f64 level given the
/// same `(k, n)`. `z = 1.96` is hardcoded.
///
...
/// `PowerResult` per sample size. Mirrors `_aggregate` (orchestration.py:106).
///
