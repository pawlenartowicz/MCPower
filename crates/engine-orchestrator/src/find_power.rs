...
impl<'s, 'c> EngineProgressSink for EngineSinkAdapter<'s, 'c> {
    // Polled every sim by the engine; O(1) atomic load on the shared token.
...
mod tests {
    ...
    use engine_core::{
        CorrectionMethod, CritValues, EstimatorSpec, OutcomeKind, ResidualDist,
    ...
    fn preflight_warns_for_overridden_exact_factor_under_sampled_scenario() {
        ...
        assert_eq!(
            w.len(),
            1,
            "exact-overridden sparse factor must warn; got: {:?}",
            w
        );
        assert!(
