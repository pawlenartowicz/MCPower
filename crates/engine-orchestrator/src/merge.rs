...
fn merge_one_power_result(prs: &[&PowerResult]) -> Result<PowerResult, OrchestratorError> {
    ...
    // Posthoc merge: pool block-by-block when all parts have the same structure.
    // If any part has no posthoc blocks, or blocks disagree in count or contrast
    // widths, fall back to empty — the host should not have mixed these.
...
/// Pool posthoc blocks across parts. Returns `vec![]` if any part has no
/// posthoc blocks, or if the block counts or per-block contrast widths disagree.
fn merge_posthoc_blocks(prs: &[&PowerResult], pooled_n_sims: u64) -> Vec<PosthocPower> {
