...
/// Pool-adjacent-violators: least-squares non-decreasing fit, unweighted.
...
fn pava(y: &[f64]) -> Vec<f64> {
    ...
    for (m, l) in means.iter().zip(&lens) {
        out.extend(std::iter::repeat(*m).take(*l));
    }
    out
}
...
/// probit fit. WLS of `Φ⁻¹(p̂ᵢ)` against `√Nᵢ` with delta-method
...
mod fit_tests {
    ...
    fn pava_cascading_merge() {
        ...
        assert_vec_close(&pava(&[0.2, 0.6, 0.5, 0.4, 0.8]), &[0.2, 0.5, 0.5, 0.5, 0.8]);
    }
    ...
    fn gate_fires_just_over_threshold() {
        ...
        match fit {
            CrossingFit::NonMonotone { max_violation } => {
                assert_close(max_violation, 0.15, 1e-12)
            }
    ...
    fn gate_precedes_crossing_and_hint() {
        ...
        match fit {
            CrossingFit::NonMonotone { max_violation } => {
                assert_close(max_violation, 0.3, 1e-12)
            }
            other => panic!("expected NonMonotone, got {other:?}"),
        }
    }
    ...
    fn probit_hint_golden_value() {
        ...
        assert_eq!(fit, CrossingFit::NotReached { n_approx: Some(352) });
    }

    #[test]
    fn probit_hint_ceils_to_cluster_atom() {
        let fit = fit_crossing(&GRID, &[30, 45, 60, 72], 100, 0.9, 30);
        assert_eq!(fit, CrossingFit::NotReached { n_approx: Some(360) });
    }
    ...
    fn probit_hint_low_power_gate_just_under_vs_just_over() {
        ...
        assert_eq!(over, CrossingFit::NotReached { n_approx: Some(263) });
    }
