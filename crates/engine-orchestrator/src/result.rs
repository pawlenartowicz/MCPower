...
pub struct PowerResult {
    ...
    /// (serde-default).
    /// Deliberate RULE-1 exception: the engine composes these English strings
    /// so the wording is single-sourced across all four ports. Hosts re-emit
    /// them verbatim and must never parse them; the text may change at any
    /// Z bump. Mirrors SampleSizeResult.grid_warnings — change together.
    #[serde(default)]
...
pub enum EstimatorExtras {
    ...
    Glm {
        ...
        /// fits — the engine half of the §5 Laplace-bias warning (the host
...
mod result_counter_tests {
    ...
    fn crossing_fit_serde_roundtrips_all_variants() {
        let variants = vec![
            ...
            CrossingFit::NotReached { n_approx: Some(330) },
    ...
    fn from_host_args_rejects_unknown_tokens() {
        ...
        assert!(SampleSizeMethod::from_host_args("grid", Some(1), None, Some("geometric")).is_err());
    }
}
