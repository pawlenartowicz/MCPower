...
pub fn single_core_find_power(
    ...
    let out = for_each_scenario(
        ...
        |idx, scenario, progress| {
            ...
            let adapter = EngineSinkAdapter::new(sample_size, offset, 1, total_sims, taken, cancel);
            ...
            *progress = adapter.outer.into_inner().unwrap_or(None);
            ...
            // Per-N power readout — mirrors find_power (twin mandate); hosts
            // pooling single-core workers rely on this for live per-N feedback.
            if let Some(p) = progress.as_deref_mut() {
...
pub fn single_core_find_sample_size(
    ...
    let out = for_each_scenario(
        ...
        |idx, scenario, progress| {
            ...
            scenario_warnings.extend(crate::find_power::factor_preflight_sample_size(
                &scenario.spec,
                &grid,
            ));
            Ok(SampleSizeResult {
