...
use engine_contract::{
    ...
    OutcomeSpec, ResidualDist, ResidualSpec, ScenarioPerturbations, SimulationContract, TestSpec,
    TestTarget,
};
...
fn factor_contract(probs: [f64; 2]) -> SimulationContract {
    ...
    let scenario = ScenarioPerturbations {
        ...
        ..ScenarioPerturbations::default() // sampled_factor_proportions defaults to false
    };
    SimulationContract {
        generation,
        design_generation,
...
fn preflight_twins_emit_identical_warnings() {
    ...
    let c = factor_contract([0.95, 0.05]);
    let cancel = CancellationToken::new();
    let multi = find_power(std::slice::from_ref(&c), 40, 50, 2137, None, &cancel).unwrap();
...
fn preflight_sample_size_twins_emit_identical_warnings() {
    ...
    .unwrap();
    let single =
        single_core_find_sample_size(&[c], 0.8, (20, 200), 50, method, 2137, None, &cancel)
            .unwrap();
    let multi_warns = &multi.scenarios[0].1.grid_warnings;
    let single_warns = &single.scenarios[0].1.grid_warnings;
    assert_eq!(
