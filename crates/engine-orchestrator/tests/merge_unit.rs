...
/// success_counts length mismatch (a part built without the raw per-target
/// counters) must be rejected in preflight, not zip-truncated against the head —
/// the WASM wire is the only place an older/partial payload can meet a full one.
#[test]
fn merge_rejects_mismatched_success_counts_length() {
    let a = ols_pr(10, vec![5, 5], vec![5, 5], 10);
    let b = ols_pr(10, vec![5], vec![5], 10); // one target's counters dropped
    assert!(matches!(
...
fn merge_pools_contrast_entries() {
    ...
    assert_eq!(
        ...
        2,
        "contrast slot must survive the merge"
    );
...
fn merge_recomputes_crossing_fit_from_pooled_counts_golden() {
    ...
    let part_a = ssr_grid(vec![
        mk(50, 30, 5),
        mk(100, 40, 10),
        mk(150, 45, 15),
        mk(200, 50, 20),
    ]);
    let part_b = ssr_grid(vec![
        mk(50, 30, 5),
        mk(100, 35, 10),
        mk(150, 40, 15),
        mk(200, 45, 20),
...
/// Asymmetric histogram-length mismatch: only ONE of the two histograms is the
/// wrong length. The guard must reject when EITHER differs (`||`), not only when
/// both do — otherwise a part missing just one per-sim joint-significance counter
/// slips through and corrupts the elementwise pool. Closes a cargo-mutants
/// survivor at merge.rs:124 (`||` → `&&`); the symmetric-length fixtures above
/// never exercised the one-wrong-one-right case.
#[test]
fn merge_rejects_asymmetric_histogram_length() {
    let mut a = ols_pr(4, vec![3, 2], vec![3, 2], 4);
    let mut b = ols_pr(4, vec![1, 4], vec![1, 4], 4);
    a.success_count_histogram_uncorrected = vec![1, 1, 2];
    a.success_count_histogram_corrected = vec![1, 1, 2];
    // Only the uncorrected histogram is the wrong length; corrected still matches.
    b.success_count_histogram_uncorrected = vec![0, 2]; // len 2, head is len 3
    b.success_count_histogram_corrected = vec![0, 2, 2]; // len 3, matches head
    let r = merge_power_results(&wrap(vec![a, b]));
    assert!(matches!(r, Err(OrchestratorError::IncompatibleMerge(_))));
}
...
/// Mismatched posthoc structure across parts must be rejected, not silently
/// dropped to empty — mirrors every sibling pooled-vector guard. Two cases:
/// a per-block contrast-count mismatch, and a block-count mismatch.
#[test]
fn merge_rejects_mismatched_posthoc_structure() {
    // Case 1: same one block, but the contrast widths differ (3 vs 2).
    let a = pr_with_posthoc(4, vec![3, 0, 1], vec![2, 0, 0]);
    let mut b = pr_with_posthoc(4, vec![2, 1], vec![1, 1]);
    if let Some(block) = b.posthoc.get_mut(0) {
        block.n_levels = 2; // C(2,2)=1 normally, but the count len (2) is what's checked
        block.ci_uncorrected = vec![Ci { lo: 0.0, hi: 1.0 }; 2];
        block.ci_corrected = vec![Ci { lo: 0.0, hi: 1.0 }; 2];
    }
    assert!(matches!(
        merge_power_results(&wrap(vec![a, b])),
        Err(OrchestratorError::IncompatibleMerge(_))
    ...
    // Case 2: differing block counts (one block vs zero).
    let a = pr_with_posthoc(4, vec![3, 0, 1], vec![2, 0, 0]);
    let b = ols_pr(4, vec![3, 2], vec![3, 2], 4); // no posthoc block
    assert!(matches!(
        merge_power_results(&wrap(vec![a, b])),
        Err(OrchestratorError::IncompatibleMerge(_))
    ));
...
/// boundary_component_counts pool elementwise against the pooled singular_n —
/// the same Σ/Σ contract as every other count vector, and the only one whose
/// pooling+rate-recompute path (merge.rs) had no direct assertion. Parts carry
/// [1,0] and [0,2] over singular_n 3+4=7 → counts [1,2], rates [1/7, 2/7].
#[test]
fn merge_pools_boundary_component_counts() {
    let mut a = mle_pr(4, 1, 3);
    let mut b = mle_pr(4, 2, 4);
    if let EstimatorExtras::Mle {
        boundary_component_counts,
        ..
    } = &mut a.estimator_extras
    {
    ...
    if let EstimatorExtras::Mle {
        boundary_component_counts,
        ..
    } = &mut b.estimator_extras
    {
        *boundary_component_counts = vec![0, 2];
    }
    let merged = merge_power_results(&wrap(vec![a, b])).unwrap();
    let EstimatorExtras::Mle {
        singular_n,
    ...
    assert_eq!(*singular_n, 7);
    assert_eq!(*boundary_component_counts, vec![1, 2]);
    assert!((boundary_rate_per_component[0] - 1.0 / 7.0).abs() < 1e-12);
    assert!((boundary_rate_per_component[1] - 2.0 / 7.0).abs() < 1e-12);
}

/// Mismatched boundary_component_counts lengths (a part built with a different
/// variance-component count) must be rejected, not summed against a shorter
/// buffer — mirrors merge_rejects_mismatched_factor_exclusion_counts_length for
/// the one pooled count vector that previously lacked a mismatch-guard test.
#[test]
fn merge_rejects_mismatched_boundary_component_counts_length() {
    let mut a = mle_pr(4, 1, 3);
    let mut b = mle_pr(4, 2, 4);
    if let EstimatorExtras::Mle {
        boundary_component_counts,
    ...
    {
        *boundary_component_counts = vec![1]; // WRONG length
    }
    assert!(matches!(
        merge_power_results(&wrap(vec![a, b])),
        Err(OrchestratorError::IncompatibleMerge(_))
...
fn merge_glm_extras_pools_baseline_singular_tau() {
    ...
    assert!(
        (*baseline_prob_realized - 0.6).abs() < 1e-12,
        "realized={baseline_prob_realized}"
    );
    ...
    assert!(
        (*tau_squared_hat_mean - 0.8).abs() < 1e-12,
        "tau_mean={tau_squared_hat_mean}"
    );
}
