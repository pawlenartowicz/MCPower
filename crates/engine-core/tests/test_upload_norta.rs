...
use engine_core::{
    CorrectionMethod, CritValues, Distribution, EstimatorSpec, OutcomeKind,
    ResidualDist, ScenarioPerturbations, SimulationSpec,
};
...
fn collect_col(spec: &SimulationSpec, n_rows: usize, base_seed: u64, col_idx: usize) -> Vec<f64> {
    ...
    (0..n_rows).map(|r| ws.x_full[(r, col_idx)] as f64).collect()
}
