...
pub fn norm_ppf(p: f64) -> f64 {
    // Coefficients from Peter J. Acklam, "An algorithm for computing the inverse normal cumulative distribution function".
    const A: [f64; 6] = [
...
/// expansion (Numerical Recipes §6.4). Accurate to ~1e-15.
...
/// Studentized-range CDF P(W ≤ q) for `k` groups and `df` error df.
///
...
impl CritValueTable {
    ...
    pub fn build_with_tukey_k(
        ...
        // standard normal (df-independent). Wald-z is the v1 logit
        // convention: see MCPower/mcpower/stats/distributions.py
        // ::compute_critical_values_logit, called from
        // MCPower/mcpower/stats/glm.py:32.
        // Mle also uses Wald-z: the LME engine produces Wald-z statistics,
        // so z² is the correct critical-value shape (df-independent).
