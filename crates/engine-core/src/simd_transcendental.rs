...
fn simd_z_mask<S: Simd, const FUSED: bool>(simd: S, eta: S::f64s) -> (S::f64s, S::m64s) {
    let neg_abs = simd.max_f64s(simd.neg_f64s(simd.abs_f64s(eta)), simd.splat_f64s(EXP_ARG_FLOOR));
...
impl<const FUSED: bool> pulp::WithSimd for SigmoidInplaceOp<'_, FUSED> {
    ...
    fn with_simd<S: Simd>(self, simd: S) {
        ...
        for x in tail.iter_mut() {
            ...
            *x = if *x >= 0.0 { 1.0 / (1.0 + z) } else { z / (1.0 + z) };
        }
    }
}
...
fn scalar_ln_unit<const FUSED: bool>(u: f64) -> f64 {
    let u = u.max(LN_U_FLOOR).min(LN_U_CEIL);
    let mut kf = 0.0f64;
    ...
    let m = if m < std::f64::consts::SQRT_2 { m } else { 0.5 * m };
...
impl<const FUSED: bool> pulp::WithSimd for PhiInplaceOp<'_, FUSED> {
    ...
    fn with_simd<S: Simd>(self, simd: S) {
        ...
        for v in head.iter_mut() {
            ...
            let mut poly =
                simd.add_f64s(simd.mul_f64s(simd.splat_f64s(ERF_A5), t), simd.splat_f64s(ERF_A4));
...
mod tests {
    ...
    fn simd_kernel_within_1ulp_of_libm() {
        ...
        assert!(p_simd_max <= 2, "SIMD-path p drifted {p_simd_max} ULP from libm");
        assert!(
    ...
    fn unfused_kernel_within_3ulp_of_libm() {
        ...
        assert!(p_simd_max <= 3, "unfused SIMD p drifted {p_simd_max} ULP from libm");
    ...
    fn exp_fill_within_1ulp_of_libm_full_domain() {
        ...
        let n = 20_003usize;
        let xs: Vec<f64> = (0..n).map(|k| -700.0 + 1400.0 * k as f64 / n as f64).collect();
        let mut emax = 0i128;
        for &x in &xs {
    ...
    fn ln_fill_within_2ulp_of_libm() {
        ...
        let n = 20_003usize;
        let lo = 9.5e-4f64;
        let us: Vec<f64> = (0..n).map(|k| lo + (1.0 - lo) * k as f64 / n as f64).collect();
        let mut smax = 0i128;
        for &u in &us {
