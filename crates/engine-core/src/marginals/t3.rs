//! t(3) PPF lookup table — knot layout is a direct port of v1's
//! `MCPower/cpp/src/distributions.cpp:688-696` and lookup at
//! `MCPower/cpp/src/data_generation.cpp:74-93`.
//!
//! v1 constants (from `MCPower/cpp/src/data_generation.hpp:125-129`):
//!   PERC_RANGE_MIN = 0.001, PERC_RANGE_MAX = 0.999 (v1's band; retightened
//!   below so support is 6.0 SD), DIST_RESOLUTION = 2048.
//!
...
//! support ≈ ±6.0 SD). v1 divided by √3, the SD of the *uncensored* t(3);
//! censoring removes ~14% of t(3)'s tail-dominated variance, so that left
//! the marginal at var ≈ 0.858 — a silent effect shrink for every
//! high-kurtosis predictor. Deliberate departure from v1 parity (v1 had the
//! same fault). Shared across all sims and all columns; build once per run.
