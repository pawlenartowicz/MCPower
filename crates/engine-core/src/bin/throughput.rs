...
//!
//! Mixed-model cases run `off` only. LME/LMM (`estimator=Mle`) are rejected by
//! the scenario gate for non-optimistic presets; the GLMM cases (`Glm`+cluster)
//! are *not* gated, but the bench holds them off-only too — a clustered fit is
//! far too heavy per fit to also pay the 3× preset triple (see `has_on_mode`).
//! Only the OLS/plain-GLM rows carry the `on` preset triple.
//!
...
use engine_core::spec::{
    ...
    EstimatorSpec, HeteroskedasticityCoeffs, OutcomeKind, ResidualDist,
...
// alias mapping every host applies). LME knobs stay None: LME rows are
// off-mode only until LME scenario support lands.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Frozen case grid — 18 cases. The OLS/GLM/LME rows mirror the cross-port
...
impl Case {
    /// Mixed-model rows are off-only. LME/LMM (`estimator=Mle`) are rejected by
    /// the engine's scenario gate; the GLMM rows (`Glm`+cluster) are *not* gated
    /// but are held off-only for parity — a clustered fit is far too heavy to
    /// also pay the 3× preset triple. So: `on` only for non-clustered, non-Mle.
    fn has_on_mode(&self) -> bool {
        self.spec.estimator != EstimatorSpec::Mle && self.spec.cluster.is_none()
    }
}
...
fn cases() -> Vec<Case> {
    ...
    let lmm_crossed = {
        ...
        s.effect_sizes[1] = 0.15;
        s.effect_sizes[2] = 0.15;
        s.cluster.as_mut().unwrap().extra_groupings.push(GroupingSpec {
    ...
    let lmm_multislope = {
        ...
        slopes.push(SlopeTerm { column: ColumnId(0), variance: 0.10, corr_with_intercept: 0.3, corr_with: vec![] });
        slopes.push(SlopeTerm { column: ColumnId(1), variance: 0.08, corr_with_intercept: 0.1, corr_with: vec![0.2] });
    ...
    vec![
        Case { id: "ols_multi",         n: 200,  n_sims: 10_000, spec: ols_multi },
        Case { id: "ols_wide",          n: 200,  n_sims: 10_000, spec: ols_wide },
        Case { id: "ols_large_n",       n: 5000, n_sims: 2_000,  spec: ols_large_n },
        Case { id: "ols_factor_inter",  n: 200,  n_sims: 10_000, spec: ols_factor_inter },
        Case { id: "glm_multi",         n: 200,  n_sims: 10_000, spec: glm_multi },
        Case { id: "glm_wide",          n: 200,  n_sims: 5_000,  spec: glm_wide },
        Case { id: "glm_large_n",       n: 5000, n_sims: 1_000,  spec: glm_large_n },
        Case { id: "glm_rare",          n: 500,  n_sims: 10_000, spec: glm_rare },
        Case { id: "lme_multi",         n: 500,  n_sims: 2_000,  spec: lme_multi },
        Case { id: "lme_many_clusters", n: 1000, n_sims: 1_000,  spec: lme_many_clusters },
        Case { id: "lme_matched_q1",     n: 480, n_sims: 1_000, spec: lme_matched_q1 },
        Case { id: "lmm_crossed",        n: 480, n_sims: 1_000, spec: lmm_crossed },        // atom 240
        Case { id: "lmm_nested",         n: 480, n_sims: 1_000, spec: lmm_nested },         // atom 80
        Case { id: "lmm_crossed_nested", n: 480, n_sims: 1_000, spec: lmm_crossed_nested }, // atom 120
        Case { id: "lmm_slope",          n: 480, n_sims: 1_000, spec: lmm_slope },          // M3 single slope
        Case { id: "lmm_multislope",     n: 480, n_sims: 1_000, spec: lmm_multislope },     // M3 two slopes
        Case { id: "glmm_intercept",     n: 480, n_sims: 1_000,   spec: glmm_intercept },     // M4 GLMM (Glm+cluster), random intercept
        Case { id: "glmm_slope",         n: 480, n_sims: 1_000,   spec: glmm_slope },         // M4 GLMM + slope, heaviest fit in grid
        Case { id: "glmm_crossed",        n: 480, n_sims: 200, spec: glmm_crossed },        // M4 dense path, k=14 (~62 fits/s)
        Case { id: "glmm_nested",         n: 480, n_sims: 100, spec: glmm_nested },         // M4 dense path, k=24 (~27 fits/s)
        Case { id: "glmm_crossed_nested", n: 480, n_sims: 50,  spec: glmm_crossed_nested }, // M4 dense path, k=30 (~13 fits/s)
    ]
}
...
fn run_row_once(
    ...
    for spec in &specs {
        results.push(run_batch_st(spec, &[case.n], n_sims, scenario_seed(), None)?);
    }
    Ok(results)
}
...
fn parse_args() -> Args {
    ...
    while let Some(a) = it.next() {
        match a.as_str() {
            ...
            "--mode" => match it.next().as_deref() {
                ...
                _ => die("--mode requires off or on"),
            },
            other => die(&format!(
                "unknown flag {other}; usage: throughput [--save] [--smoke] [--list] [--dump-cases] [--case <id>] [--mode off|on]"
            )),
        }
    }
    if args.dump_cases
        && (args.save || args.smoke || args.list || args.case_filter.is_some() || args.mode_filter.is_some())
    {
    ...
    if args.save && args.mode_filter.is_some() {
        die("--save with --mode would overwrite the full baseline with a partial run; drop one");
    }
...
fn main() {
    ...
    if args.dump_cases {
        let dumps: Vec<CaseDump> = all
            ...
            .collect();
        println!("{}", serde_json::to_string(&dumps).expect("cases serialize"));
        return;
    }
...
/// Modes a row runs under the optional `--mode` filter: the case's natural
/// modes (`off` / `off`+`on`) intersected with the filter. Empty for an
/// off-only case under `--mode on` — the caller skips the row.
fn row_modes(case: &Case, mode_filter: Option<&str>) -> Vec<&'static str> {
    let natural: &[&str] = if case.has_on_mode() { &["off", "on"] } else { &["off"] };
    natural
        .iter()
        .copied()
        ...
        for mode in &row_modes(case, mode_filter) {
            match run_row_once(case, mode, 4) {
                Ok(results) => {
                    ...
                    println!("{:<18} {:<4} n={:<5} n_sims=4 conv={conv:.2} ok", case.id, mode, case.n);
                }
                Err(e) => {
                    failed = true;
...
fn run_timed(selected: &[&Case], save: bool, mode_filter: Option<&str>) {
    ...
    let baseline_record = |case_id: &str, mode: &str| -> Option<&Record> {
        baseline
            .as_ref()
            .and_then(|b| b.records.iter().find(|r| r.case_id == case_id && r.mode == mode))
    };

    println!(
        "{:<18} {:<4} {:>5} {:>7} {:>10} {:>12} {:>6} {:>7} {:>7} {:>7} {:>6}  vs_baseline",
        "case", "mode", "n", "n_sims", "elapsed_s", "fits_per_sec", "conv", "power", "ev/fit", "ev/f_cv", "pin"
    );
    ...
    for case in selected {
        for mode in &row_modes(case, mode_filter) {
            ...
            let n_sims = if *mode == "on" { case.n_sims / 2 } else { case.n_sims };
            ...
            let vs = match baseline_record(case.id, mode) {
                Some(base) if base.fits_per_sec > 0.0 => {
                    ...
                    if base_has_sig
                        && (base.k_unc != k_unc || base.k_conv != k_conv || base.sig_hash != sig_hash)
                    {
