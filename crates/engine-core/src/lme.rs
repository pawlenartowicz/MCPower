...
use crate::ols::{OlsScratch, PANEL_ROWS};
...
/// Initial θ bracket — log-spaced. The upper bound matches v1's reach:
/// v1 (`MCPower/cpp/src/lme_solver.cpp:367`) searches λ² ∈ [0, 1e6] ⇒
/// θ ∈ [0, 1000]. Capping at θ=1e2 would truncate v1's domain and trigger
/// high-τ rebracket retries on cases v1 handles in its main loop.
...
pub struct LmeScratch<'w> {
    ...
    pub sigma_sq: f64,
    /// Brent scalar state (8 fields). Built inline at the call site as
    ...
    pub brent_d: &'w mut f64,
    pub brent_e: &'w mut f64,
...
// REML form (matches MCPower/cpp/src/lme_solver.cpp:151-158, the REML branch
// of `profiled_deviance_q1`). v1 parametrises by lam_sq = θ²; the inner loop
...
// Golden-section + parabolic-interpolation constants (Numerical Recipes §10.3).
...
/// Scientific Computing", 3rd ed. (Cambridge, 2007), §10.3 "Brent's Method in
/// One Dimension". The algorithm structure and variable names below follow
/// NR §10.3 closely.
///
/// # Caller contract
...
/// here — the workspace's `lme_brent_d` / `lme_brent_e` slots are vestigial.
/// Stack scalars cost zero allocations (the workspace slots existed to
/// document the no-per-call-alloc invariant). Leave the workspace fields
/// as-is for any future caller that
/// wants to inspect the Brent trajectory across sims.
pub fn lme_fit<'a>(
    _x: MatRef<'_, f32>,
    _y: &[f32],
    _cluster_ids: &[u32],
    ...
    for i in 0..p {
        let d = scratch.xtvix_factor[(i, i)].abs();
        if d > max_diag {
            max_diag = d;
        }
        if d < min_diag {
    ...
    let rank_deficient = max_diag <= 0.0 || min_diag < EPS_RANK * max_diag;
    ...
    // are ‖L⁻¹ e_j‖². Mirrors ols.rs:425-462 verbatim.
    for &tj in target_indices.iter() {
        let tj = tj as usize;
        if tj >= p {
            continue;
...
mod tests {
    ...
    fn lme_add_rows_panel_matches_scalar_reference() {
        ...
        let mut ws = SimWorkspace::new(
            ...
            Some(&engine_contract::ClusterSpec::intercept_only(
                engine_contract::ClusterSizing::FixedClusters { n_clusters: k as u32 },
                0.25,
            )),
        );
    ...
    fn profiled_deviance_overwrites_state() {
        ...
        let mut ws = SimWorkspace::new(
            ...
            0,
            0,
            Some(&engine_contract::ClusterSpec::intercept_only(engine_contract::ClusterSizing::FixedClusters { n_clusters: n_clusters as u32 }, 0.25)),
        );
        ws.reset_lme_suff_stats();
        {
            let mut s = LmeSuffStats {
    ...
    fn lme_fit_converges_with_valid_boundary() {
        ...
        let mut ws = SimWorkspace::new(
            ...
            0,
            0,
            Some(&engine_contract::ClusterSpec::intercept_only(engine_contract::ClusterSizing::FixedClusters { n_clusters: K as u32 }, 0.25)),
        );
        ws.reset_lme_suff_stats();
        {
            let mut s = LmeSuffStats {
    ...
    fn lme_fit_detects_tau_zero_boundary() {
        ...
        let y_f64: Vec<f64> = (0..N)
            .map(|i| {
                let row_signal = beta_true[0] + beta_true[1] * x_f64[(i, 1)] + beta_true[2] * x_f64[(i, 2)];
    ...
    fn lme_truth_start_matches_cold_minimum_with_fewer_evals() {
        ...
        assert_eq!(warm.boundary_hit, cold_boundary, "boundary flags must agree");
    ...
    fn joint_wald_ols_fallback_returns_finite_chi_sq() {
        ...
        let y: Vec<f32> = (0..N)
            .map(|i| (0.5 + 0.4 * x[(i, 1)] as f64 - 0.3 * x[(i, 2)] as f64 + ((i as f64) * 0.011).sin() * 0.05) as f32)
            .collect();
        let cluster_ids: Vec<u32> = (0..N).map(|i| (i % K) as u32).collect();
