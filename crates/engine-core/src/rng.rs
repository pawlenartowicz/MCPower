...
// The normal-inverse breakpoint/floor and the minimax polynomial coefficients
// below are written to full decimal precision to document the derived f32 value
// (Acklam/minimax fit, provenance for the bit-identical generation invariant).
// The `_f32` suffix rounds each to the same bits regardless, so the extra digits
...
impl SimRng {
    ...
    pub fn next_categorical(&mut self, probs: &[f64]) -> usize {
        ...
        debug_assert!(
            ...
            "next_categorical: negative probability"
        );
...
//
// Form — the sanctioned one-`ln` fallback.
...
mod tests {
    ...
    fn fill_uniform_column_bit_matches_scalar() {
        ...
        for i in 0..n {
            assert_eq!(
                out[i].to_bits(),
                u32_to_unit_f32(words[i]).to_bits(),
                "draw {i}"
            );
        }
    }
