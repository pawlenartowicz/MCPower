...
impl LmmSuffStats {
    ...
    pub fn add_rows_multi(
        ...
        for row in 0..x.nrows() {
            ...
            for &a in &gid[..n_g] {
                let scol = self.s.col_mut(a).try_as_col_major_mut().unwrap().as_slice_mut();
                for j in 0..self.m {
                    scol[j] += self.w_buf[j];
                }
            }
            for j in 0..self.m {
                let wj = self.w_buf[j];
                let ccol = self.c.col_mut(j).try_as_col_major_mut().unwrap().as_slice_mut();
                for i in j..self.m {
            ...
            if self.groupings.k_crossed() > 0 {
                ...
                for bi in 0..n_g {
                    ...
                    if b >= kf {
                        ...
                        if slope {
                            for (d, &sc) in
                                self.groupings.primary_slope_cols.iter().enumerate()
                            {
                                let z = self.w_buf[sc];
            ...
            if self.groupings.primary_q > 1 {
                ...
                for (k, &sc) in self.groupings.primary_slope_cols.iter().enumerate() {
                    ...
                    let scol_mut =
                        self.s.col_mut(scol).try_as_col_major_mut().unwrap().as_slice_mut();
                    for j in 0..self.m {
...
pub fn reml_deviance(theta: &[f64], suff: &LmmSuffStats, fit: &mut LmmFitScratch) -> f64 {
    ...
    for f in 0..g.n_primary {
        ...
        if slope {
            ...
            for (d, sc) in s_cols.iter_mut().enumerate().take(q) {
                *sc = suff.s.col(d * n_prim + f).try_as_col_major().unwrap().as_slice();
            }
            for r in 0..q {
    ...
    {
        let bt =
            faer::MatRef::from_column_major_slice(&fit.bt[..t_dim * w_tot], t_dim, w_tot);
...
pub fn fit_lmm(
    ...
    for i in 0..p {
        let di = fit.factor[(i, i)].abs();
        max_d = max_d.max(di);
        min_d = min_d.min(di);
    }
    let degenerate = !dev.is_finite() || max_d <= 0.0 || min_d < EPS_RANK * max_d;
    if !ok || degenerate {
...
mod tests {
    ...
    /// Deterministic pseudo-data (Numerical Recipes LCG), uniform in (−1, 1).
    ...
    fn shipped_workspace(x: &Mat<f32>, y: &[f32], ids: &[u32], n_clusters: u32) -> SimWorkspace {
        let mut ws = SimWorkspace::new(
            ...
            x.ncols() - 1,
            0,
            Some(&engine_contract::ClusterSpec::intercept_only(engine_contract::ClusterSizing::FixedClusters { n_clusters }, 0.25)),
        );
    ...
    fn multi_dataset(
        with_nested: bool,
        n_blocks: usize,
    ) -> (Mat<f32>, Vec<f32>, Vec<u32>, Vec<Vec<u32>>, engine_contract::ClusterSpec) {
        ...
        for i in 0..n {
            ...
            y[i] = (0.5 + 0.4 * x1 - 0.2 * x2
                ...
                + if with_nested { u_n[eids[1][i] as usize] } else { 0.0 }
    ...
    fn nested_regime_b_deviance_matches_brute_force() {
        use engine_contract::{ClusterSizing, ClusterSpec, GroupingRelation, GroupingSpec};
        let mut cluster = ClusterSpec::intercept_only(
            ClusterSizing::FixedSize { cluster_size: 8 },
            0.25,
        );
        cluster.extra_groupings.push(GroupingSpec {
            relation: GroupingRelation::NestedWithin { n_per_parent: 2 },
        ...
        for i in 0..n {
            ...
            x[(i, 0)] = 1.0;
            x[(i, 1)] = x1 as f32;
            y[i] = (0.5 + 0.4 * x1 + u_p[pid[i] as usize] + u_c[cid[i] as usize]
                + 0.8 * lcg(&mut st)) as f32;
        }
        let g = LmmGroupings::from_cluster_spec(&cluster, n, &[]);
    ...
    fn two_crossed_factors_deviance_matches_brute_force() {
        use engine_contract::{ClusterSizing, ClusterSpec, GroupingRelation, GroupingSpec};
        let mut cluster = ClusterSpec::intercept_only(
            ClusterSizing::FixedClusters { n_clusters: 3 },
            0.25,
        );
        for k in [4u32, 2u32] {
            cluster.extra_groupings.push(GroupingSpec {
        ...
        for i in 0..n {
            ...
            x[(i, 0)] = 1.0;
            x[(i, 1)] = x1 as f32;
            y[i] = (0.5 + 0.4 * x1 + u_p[pid[i] as usize] + u_a[e0[i] as usize]
                + u_b[e1[i] as usize] + 0.8 * lcg(&mut st)) as f32;
        }
        let g = LmmGroupings::from_cluster_spec(&cluster, n, &[]);
        let mut suff = LmmSuffStats::with_groupings(2, g);
    ...
    fn zero_crossed_variance_pins_only_that_component() {
        ...
        let i_cl = 3usize;
        let mut cluster = ClusterSpec::intercept_only(
            ClusterSizing::FixedClusters { n_clusters: s_cl as u32 },
            0.25,
        );
        cluster.extra_groupings.push(GroupingSpec {
            relation: GroupingRelation::Crossed { n_clusters: i_cl as u32 },
            tau_squared: 0.0,
            slopes: vec![],
        });
        ...
        for i in 0..n {
            ...
            x[(i, 0)] = 1.0;
            x[(i, 1)] = x1 as f32;
            let e = if (i / cluster.atom()) % 2 == 0 { 0.8 } else { -0.8 };
        ...
        assert!(ws.theta[0] > PIN_THETA, "primary component must stay interior");
    ...
    fn truth_start_matches_blind_on_crossed_nested_bytes() {
        ...
        for j in 0..3 {
            ...
            let d = (a - b).abs();
            assert!(d <= 1e-5 || d <= 1e-4 * a.abs().max(b.abs()), "β[{j}]: {a} vs {b}");
        }
    }
    ...
    fn multislope_dataset() -> (Mat<f32>, Vec<f32>, Vec<u32>) {
        ...
        for i in 0..n {
            ...
            x[(i, 1)] = x1 as f32;
            x[(i, 2)] = x2 as f32;
            y[i] = (0.5 + 0.4 * x1 + 0.2 * x2 + u0[c] + u1[c] * x1 + u2[c] * x2
                + 0.8 * lcg(&mut st)) as f32;
        }
        (x, y, ids)
    }
    ...
    fn slope_fit_converges_interior() {
        ...
        assert!((0.05..0.6).contains(&ws.fit.betas[1]), "slope {}", ws.fit.betas[1]);
    ...
    fn multislope_fit_converges_interior() {
        ...
        assert!((0.2..0.9).contains(&ws.fit.betas[0]), "intercept {}", ws.fit.betas[0]);
        assert!((0.2..1.1).contains(&ws.fit.betas[1]), "slope x1 {}", ws.fit.betas[1]);
        assert!((0.0..0.7).contains(&ws.fit.betas[2]), "slope x2 {}", ws.fit.betas[2]);
        assert!(ws.fit.betas[1] > ws.fit.betas[2], "x1 slope must exceed x2 slope");
    ...
    fn zero_slope_variance_pins_slope_component() {
        ...
        let mut ws = LmmWorkspace::with_groupings(2, LmmGroupings::from_cluster_spec(
            &engine_contract::ClusterSpec {
                sizing: engine_contract::ClusterSizing::FixedClusters { n_clusters: nc as u32 },
                tau_squared: 0.25,
                slopes: vec![engine_contract::SlopeTerm {
                    column: engine_contract::ColumnId(0),
                    variance: 0.1,
        ...
        let fit = fit_lmm(&mut ws, &[1], None);
        assert!(fit.converged);
        assert!(ws.theta[2] == 0.0, "slope λ₁₁ must pin to exactly 0, got {:e}", ws.theta[2]);
        assert!(fit.pinned_components & 0b10 != 0, "slope component bit set");
        assert!(ws.theta[0] > PIN_THETA, "intercept component must stay interior");
        assert!(ws.theta[0] < THETA_HI, "intercept component must be off the box bound");
    }
    ...
    fn composed_deviance_matches_brute_force() {
        ...
        suff.add_rows_multi(x.as_ref(), &y, &pid, &[iid.clone()]); // item ids as the single extra grouping
