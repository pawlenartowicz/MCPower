...
// variables tagged as continuous-normal (matches v1 `scenarios.py:368-372`).
...
/// `scenario.new_distributions`. Mirrors v1 `scenarios.py:366-373` plus the
/// pin rule (default = scenarios decide; explicitly set = pinned). `pinned`
...
/// `scenario.residual_df` regardless (no model-level df). Mirrors v1
/// `simulation.py:601-611` plus the pin rule.
...
mod tests {
    ...
    fn perturb_var_types_skips_pinned_normal() {
        ...
        assert_eq!(out_pinned[1], Distribution::Normal, "pinned normal must survive");
    ...
    fn pick_residual_never_swaps_pinned_or_non_normal() {
        ...
        for _ in 0..100 {
            ...
            let non_normal =
                pick_residual(&scenario, ResidualDist::HighKurtosis, false, &mut rng);
            assert_eq!(
                non_normal,
