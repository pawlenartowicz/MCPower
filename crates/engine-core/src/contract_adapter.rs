...
) -> (Vec<Distribution>, Vec<bool>, Vec<f64>, Vec<i32>, Vec<f64>, Vec<Option<bool>>, u32, u32) {
...
mod tests {
    ...
    use engine_contract::{
        ...
        GenerationSpec, LmeScenarioPerturbations, OutcomeKind, OutcomeSpec,
        PosthocSpec, ResidualSpec, ScenarioPerturbations, SimulationContract, SlopeTerm, TestSpec,
        TestTarget, UploadedFrame,
    };
    ...
    fn drop_leading_continuous_contract() -> SimulationContract {
        SimulationContract {
            generation: GenerationSpec {
                columns: vec![
                    ColumnSpec::Synthetic { kind: SyntheticKind::Normal, pinned: false },
                    ColumnSpec::Synthetic { kind: SyntheticKind::Normal, pinned: false },
                ],
                correlations: Correlations::Identity,
                cluster: None,
            ...
            design_generation: DesignSpec {
                terms: vec![
                    DesignTerm::Const,
                    DesignTerm::Direct { column: ColumnId(0) },
                    DesignTerm::Direct { column: ColumnId(1) },
                ],
            },
            outcome: OutcomeSpec {
                kind: OutcomeKind::Continuous,
                ...
                residual: ResidualSpec { distribution: ResidualDist::Normal, pinned: false },
                heteroskedasticity_driver: None,
            },
            design_test: Some(DesignSpec {
                // keep x2 (ColumnId(1)), drop the leading x1.
                terms: vec![DesignTerm::Const, DesignTerm::Direct { column: ColumnId(1) }],
            }),
            estimator: EstimatorSpec::Ols,
            test: TestSpec {
    ...
    fn report_indices_name_reduced_term_position_when_leading_term_dropped() {
        ...
        assert_eq!(report_targets, vec![1], "report index = reduced term position");
    ...
    fn report_indices_handle_factor_target_after_dropped_leading_continuous() {
        ...
        let c = SimulationContract {
            ...
            design_generation: DesignSpec {
                terms: vec![
                    DesignTerm::Const,
                    DesignTerm::Direct { column: ColumnId(0) },
                    DesignTerm::DummyOf { column: ColumnId(1), level_index: 1 },
                    DesignTerm::DummyOf { column: ColumnId(1), level_index: 2 },
                ],
            },
            outcome: OutcomeSpec {
                kind: OutcomeKind::Continuous,
        ...
        let s = contract_to_simulation_spec(&c).unwrap();
        assert_eq!(s.target_indices, vec![2, 3], "kernel read = factor dummy columns");
        let (report_targets, _) = report_targets_and_contrasts(&c);
        assert_eq!(report_targets, vec![1, 2], "report = reduced term positions");
    ...
    fn continuous_interaction_maps_to_appended_kernel_column() {
        use engine_contract::{
            ColumnId, CorrectionMethod, DesignSpec, EstimatorSpec, GenerationSpec,
        ...
        let c = SimulationContract {
            ...
            design_generation: DesignSpec {
                terms: vec![
                    ...
                    DesignTerm::Direct { column: ColumnId(0) },
                    DesignTerm::Direct { column: ColumnId(1) },
                    DesignTerm::DummyOf { column: ColumnId(2), level_index: 1 },
                    DesignTerm::DummyOf { column: ColumnId(2), level_index: 2 },
                ],
            },
            outcome: OutcomeSpec {
                kind: OutcomeKind::Continuous,
