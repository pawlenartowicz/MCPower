...
pub fn glm_irls_fit<'a>(
    ...
    // iteration's post-accept recompute (step 7) instead of recomputing
    // X·β at the top of every iteration.
    match beta_start {
        // Truth start: β ← β₀, η = X·β₀ computed once (O(np)). The η loop
        // matches step 7's summation order (column-sweep axpy) so a warm fit
    ...
    for iter in 0..=MAX_IRLS_ITERS {
        // (1) η = X · β is already in `irls_eta`: seeded to 0 (β = 0) or the
        // truth start before the loop, then refreshed by step 7 after each β
        // accept. Reusing it here is bit-identical to recomputing X·β (same β,
        // same summation order).

        // (2) p, W, and Σ log1pexp(η) in one fused vectorized pass; the working
        ...
        // (2b) Adaptive early exit on |Δdeviance|, where deviance is the
        ...
        // (3) WX = W∘X into irls_wx (column-major, mirrors x_f64), then
        ...
        triangular::matmul(
            irls_xtwx.rb_mut(), BlockStructure::TriangularLower, Accum::Replace,
            x_ref.transpose(), BlockStructure::Rectangular,
            wx_ref, BlockStructure::Rectangular,
            1.0, Par::Seq,
        );
        matmul(
            MatMut::from_column_major_slice_mut(&mut irls_xtwz[..p], p, 1),
            Accum::Replace, wx_ref.transpose(),
            MatRef::from_column_major_slice(&irls_z[..n], n, 1),
            1.0, Par::Seq,
        );

        // (4) Cholesky of X'WX on the lower triangle. faer's high-level API
        ...
        // (5) Solve β_new = L⁻ᵀ L⁻¹ · X'Wz using chol.solve_in_place.
        ...
        // (7) Accept β_new unconditionally and compute new deviance.
        //
        // v1 parity note: v1 (cpp/src/glm_solver.cpp) accepts β_new without
        // step-halving — it simply assigns `beta = beta_new`. Step-halving
        // was added here in an earlier draft but it deviates from v1's
        // reference behaviour and causes systematic power drift at small N
        // (measured ~3.5% at N=50). The DEVIANCE_TOL early-exit
        // and MAX_IRLS_ITERS cap are sufficient divergence guards.
        ...
        // The new η's deviance is checked at the top of the next pass (step 2b).

        // (8) BETA_CAP divergence guard at iter ≥ 3. Fires before the next
    ...
    if converged {
        ...
        let saturated = irls_w[..n].iter().filter(|&&w_i| w_i < SATURATION_W).count();
...
mod tests {
    ...
    fn glm_converges_on_separable_signal() {
        ...
        for &t in fit.t_sq.iter() {
            assert!(t.is_finite() && t >= 0.0, "z² must be finite, non-negative");
        }
    }
