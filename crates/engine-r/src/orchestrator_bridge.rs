...
use engine_orchestrator::{
    power_result_to_host, sample_size_result_to_host, HostValue, PowerResult,
    SampleSizeMethod, SampleSizeResult, ScenarioResult,
};
use extendr_api::error::Result;
use extendr_api::prelude::*;

use crate::progress::{orchestrator_err, run_with_progress};
...
pub fn find_power(
    ...
    let result = run_with_progress(progress, &cancel, |sink, cancel| {
...
pub fn find_sample_size(
    ...
    let method =
        SampleSizeMethod::from_host_args(method, by.map(|v| v as usize), by_kind, mode)
            .map_err(Error::Other)?;
