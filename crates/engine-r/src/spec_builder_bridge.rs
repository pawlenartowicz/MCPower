//! extendr bindings for spec building and formula/assignment parsing; mirrors engine-py's spec_builder_bridge.
...
pub fn parse_assignments(input: &str, kind: &str, known_json: &str) -> Result<String> {
    ...
    // Convert to serializable wrapper (mirrors engine-py's manual dict construction).
