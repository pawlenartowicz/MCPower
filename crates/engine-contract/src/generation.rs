...
mod tests {
    ...
    fn factor_synthetic_sampled_defaults_to_none_when_absent() {
        ...
        #[derive(serde::Serialize)]
        enum OldColumnSpec {
            FactorSynthetic { n_levels: u32, proportions: Vec<f64> },
        }
        ...
        assert!(matches!(
            back,
            ColumnSpec::FactorSynthetic { sampled_proportions: None, .. }
        ));
    }
    ...
    /// §2.1 growth: a payload WITHOUT the new keys (today's wire shape from every
    ...
    fn nested(n_per: u32, tau: f64) -> GroupingSpec {
        GroupingSpec {
            relation: GroupingRelation::NestedWithin { n_per_parent: n_per },
            tau_squared: tau,
            slopes: vec![],
        }
    }
    ...
    fn cluster_spec_atom_multiplies_extra_groupings() {
        let mut spec = ClusterSpec::intercept_only(
            ClusterSizing::FixedClusters { n_clusters: 20 },
            0.25,
        );
        ...
        let mut b = ClusterSpec::intercept_only(
            ClusterSizing::FixedSize { cluster_size: 6 },
            0.25,
        );
        b.extra_groupings.push(nested(3, 0.05));
        assert_eq!(b.atom(), 18); // §2.2: primary atom · n_per_parent
    }
    ...
    fn crossed_layout_full_factorial_when_s_equals_i() {
        ...
        for i in 0..atom {
            ...
            assert!(seen.insert(pair), "duplicate pair {pair:?} within one block");
        }
    ...
    fn nested_regime_a_layout_and_balance() {
        let mut spec = ClusterSpec::intercept_only(
            ClusterSizing::FixedClusters { n_clusters: 2 },
            0.25,
        );
        spec.extra_groupings.push(nested(3, 0.05));
        ...
        for i in 0..n {
            ...
            assert_eq!(c / 3, spec.sizing.cluster_of_row(i), "child {c} not in its parent");
            counts[c] += 1;
        }
        assert!(counts.iter().all(|&k| k == n / 6), "unbalanced at atom multiple: {counts:?}");
    }
