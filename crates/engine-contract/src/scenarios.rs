...
pub struct ScenarioPerturbations {
    pub name: String,
    /// Scenario contribution to the β-jitter SD, additive with the
    /// model-level baseline (effective SD = `(baseline + scenario).max(0)`).
    /// Per observation, each effect coefficient is re-drawn with
    /// SD = effective·|βⱼ|; binary outcomes additionally jitter the intercept
    /// on the log-odds scale with SD = the effective value. 0.0 = off.
