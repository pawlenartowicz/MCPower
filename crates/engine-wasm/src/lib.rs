...
impl ProgressEmitter for JsEmitter {
    /// Forwards the serialized `ProgressEvent` JSON to the JS callback; a failed
    /// call is ignored (cancellation is host-driven via `worker.terminate()`).
...
/// One worker's share of a power run at one `sample_size`; `spec_json` is an
/// `AppSpec`. Returns the per-worker `ScenarioResult<PowerResult>` JSON;
/// `on_progress` receives `ProgressEvent` JSON strings.
#[wasm_bindgen]
pub fn find_power(
    spec_json: &str,
    sample_size: usize,
...
/// One worker's share of a Grid sample-size search; inputs are JSON-encoded.
/// Returns the per-worker `ScenarioResult<SampleSizeResult>` JSON.
#[wasm_bindgen]
...
/// Pool per-worker `ScenarioResult<PowerResult>` parts (a JSON array) into one
/// merged result JSON (main thread).
#[wasm_bindgen]
...
/// Pool per-worker `ScenarioResult<SampleSizeResult>` parts (a JSON array) into
/// one merged result JSON (main thread).
#[wasm_bindgen]
...
/// The index-only `EffectSkeleton` (β-column aligned) as JSON for `spec_json`.
/// Consumers render result names from this + their own factor-label store — the
/// same single-sourced layout the Py/R bridges return, so no host re-derives
/// factor expansion. A run's `target_indices` index into the returned array
/// directly (index 0 = intercept).
#[wasm_bindgen]
...
/// Effect-recovery preview fitted from the spec's uploaded data, as JSON: the
/// `EffectsFromData` object `{ effects, cluster_icc, baseline_probability }`
/// (not a bare array). The estimator follows the spec family (OLS/GLM/MLE);
/// invalid input returns a `JsError`.
#[wasm_bindgen]
pub fn get_effects_from_data(spec_json: &str) -> Result<String, JsError> {
    ...
    let recovered = app_get_effects(&spec).map_err(err)?;
    serde_json::to_string(&recovered).map_err(err)
}
...
pub fn bench_run_pass(
    ...
    let batch = engine_core::batch::run_batch_st(&spec, &[n], n_sims, seed, None).map_err(err)?;
