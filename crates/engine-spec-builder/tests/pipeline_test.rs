...
fn reference_contrast_dedups_against_coefficient_test_marginal() {
    ...
    // is preserved.
    assert_eq!(s.target_indices.len(), 1, "group[B] marginal deduped to one");
    assert_eq!(s.contrast_pairs.len(), 1, "non-reference contrast kept");
}
...
fn scenario_residual_df_unset_with_df_consuming_pool_is_rejected() {
    ...
    spec.scenarios = vec![sc];
    let err =
        build_linear_spec(&spec).expect_err("df-consuming pool without df must be rejected");
    match err {
