...
fn intercept_suppression_rejected() {
    for f in [
        ...
        assert!(
            matches!(parse_formula(f), Err(SpecError::RandomInterceptSuppressionUnsupported)),
            "expected suppression error for {f}, got {:?}", parse_formula(f)
        );
    }
}
...
fn implicit_intercept_slope_equals_explicit() {
    ...
    assert_eq!(
        imp.random_effects,
        vec![RandomEffect::Slope { group: "g".into(), vars: vec!["x".into()] }]
    );
}
...
fn implicit_intercept_multivar_slope() {
    ...
    assert_eq!(
        p.random_effects,
        vec![RandomEffect::Slope { group: "g".into(), vars: vec!["x".into(), "z".into()] }]
    );
}
...
fn explicit_intercept_forms_unchanged() {
    ...
    assert_eq!(
        ...
        vec![RandomEffect::Intercept { group: "g".into(), parent: None }]
    );
    assert_eq!(
