...
pub fn build_linear_contract_with_skeleton(
    ...
    // Build the full predictor name order (non-factors then factors) for upload matching.
...
/// table. Non-factor names map to their 0-based formula position; factor names
/// map to `n_non_factor + factor_index` — the same column ordering `build_columns`
/// emits (non-factors first, then factors). Mirrors the name resolution used by
...
///
/// When a `test_formula` is set, the contract's `design_test.terms` reference
/// the SAME `generation.columns` entries as `design_generation` does — we
/// don't re-emit columns. Just remap each term in the test formula to its
/// generation column ID by name.
...
mod tests {
    ...
    fn cluster_level_vars_resolve_to_columns() {
        ...
        spec.predictors = vec![
            ...
            PredictorSpec { name: "x1".into(), kind: VarKind::Normal, pinned: false },
        ];
        spec.effects = vec![
            EffectAssignment { name: "group[B]".into(), size: 0.2 },
            EffectAssignment { name: "group[C]".into(), size: 0.4 },
            EffectAssignment { name: "x1".into(), size: 0.5 },
        ];
        spec.targets = vec!["x1".into()];
        ...
        assert!(matches!(c.generation.columns[0], ColumnSpec::Synthetic { .. }));
        assert!(matches!(
    ...
    fn code_tables_match_decode_arms() {
        ...
        let parse = |s: &str| {
            serde_json::from_str::<std::collections::BTreeMap<String, i32>>(s).unwrap()
        };
    ...
    fn lme_scenario_knob_accepted_through_build_contract_with_skeleton() {
        ...
        let cluster = ClusterSpec::intercept_only(
