...
//!
//! Mirrors the Python `_AssignmentParser` (`ports/py/mcpower/spec/parsers.py`):
...
/// Which assignment grammar a `name=value` string is parsed under.
...
/// Assignment target: a single predictor `Name`, or a `Pair` of names keying a
/// correlation (keys stored sorted a ≤ b).
...
/// Parse result: successfully parsed `items`, plus per-item `errors` collected
...
/// Name pools an assignment LHS is validated against: bare `predictors` and the
/// `interaction_terms` derived from the formula.
...
// Split on top-level commas. Err on unbalanced parens — the host splitters
...
// into `(var_type, VarTypeParams)`. Error wording and bare-form defaults match
// the host `_parse_variable_type_value`; the factor-level ceiling is single-
// sourced from the embedded config. Returns the per-value diagnostic string on
// failure (the caller prefixes the predictor name).
