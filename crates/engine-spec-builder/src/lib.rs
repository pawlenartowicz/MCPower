...
//!  * Single source for every port — inputs lower to a `SimulationSpec` here,
//!    once; no port reimplements the lowering, so a change lands for all of them.
//!  * Column ordering (non-factors first, then factors) is owned by
//!    `variables.rs`; every other module cites it rather than re-deriving it.
...
pub use project_contract::{
    ...
    build_linear_contract_with_skeleton, dist_codes_json, re_dist_codes_json, residual_codes_json,
};
