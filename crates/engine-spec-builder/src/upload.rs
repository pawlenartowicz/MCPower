...
/// numerics. Safe: label strings only decode which numeric code maps to
...
///
/// # Errors
/// - [`SpecError::CorrelationNotPsd`]: the user-explicit overlay leaves the
///   combined (measured + overlay) matrix non–positive-semi-definite. The
///   measured block alone is always repaired to PSD first.
...
mod correlation_tests {
    ...
    fn ridge_shrink_repairs_indefinite() {
        ...
        assert!(
            !crate::correlation::is_psd(&r, 3),
            "fixture must start indefinite"
        );
        let repaired = ridge_shrink_to_psd(&r, 3);
        assert!(
            crate::correlation::is_psd(&repaired, 3),
            "repaired must be PSD"
        );
