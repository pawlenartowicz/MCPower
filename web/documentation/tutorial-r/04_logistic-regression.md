---
title: "Power Analysis for Logistic Regression in R"
description: "Power analysis for logistic regression in R: GLM with a binary outcome, baseline probability, and sample size with MCPower."
---
# Logistic regression
...
regression does not: the baseline event rate. `model$set_baseline_probability(0.3)`
...
model <- MCPower$new("recovered ~ treatment + age", family = "logit")
...
```r
model <- MCPower$new("recovered ~ treatment + age", family = "probit")
model$set_variable_type("treatment=binary")
model$set_baseline_probability(0.3)
model$set_effects("treatment=0.5, age=0.3")
result <- model$find_power(sample_size = 300, target_test = "all", verbose = FALSE)
...
```r
model <- MCPower$new("visits ~ treatment + age", family = "poisson")
model$set_variable_type("treatment=binary")
