---
title: "Power Analysis for ANOVA & Post-Hoc Tests in R"
description: "Power analysis for ANOVA and post-hoc contrasts in R: omnibus F test, pairwise comparisons, and Tukey HSD with MCPower."
---
# ANOVA & post-hoc
...
Define the formula and mark the grouping variable as a factor with `model$set_variable_type("dose_group=(factor,0.34,0.33,0.33)")`. The `(factor, p1, p2, …)` notation sets group proportions that must sum to 1. Without uploaded data, levels are integer-labelled starting at 1 (level 1 = reference). With uploaded data the labels come from the column's distinct values.

Set effects as standardised mean differences vs the reference group using bracket notation: `model$set_effects("dose_group[2]=0.5, dose_group[3]=0.8")`. Effect-size benchmarks for factor predictors: small 0.20, medium 0.50, large 0.80.
...
Other correction methods (`bonferroni`, `holm`, `bh`) also apply per-family — pass e.g. `correction = "bonferroni"` to `model$find_power`.
...
model <- MCPower$new("score ~ treatment + region")
