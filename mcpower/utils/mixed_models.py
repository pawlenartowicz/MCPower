...
def _lme_analysis_wrapper(
    ...
    This is the public API for mixed model analysis. It provides abstraction
    for future implementations (e.g., custom C++ backend).
    ...
    Args:
        ...
        backend: "statsmodels" (only option for now)
        verbose: Return detailed diagnostics

    Returns:
        When verbose=False (backward compatible):
            np.ndarray: [F_significant, uncorrected..., corrected...] or None if failed
        When verbose=True:
            Dict with:
...
def _lme_analysis_statsmodels(
    ...
    try:
        ...
        raise ImportError("statsmodels required for mixed models: pip install mcpower[mixed]") from e
    ...
    try:
        ...
        if at_boundary:
            ...
            if np.isnan(lr_stat) or lr_stat < 0 or not np.isfinite(lr_stat):
                ...
                lr_pvalue = 1 - chi2.cdf(lr_stat, df) if df > 0 else 1.0
