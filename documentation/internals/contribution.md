...
Each port has its own build step. The project `CLAUDE.md` at the repo root holds
the authoritative build commands for each port (Rust workspace check, Python
`maturin develop`, R `CMD INSTALL`, `pnpm dev` for the app). Start there before
