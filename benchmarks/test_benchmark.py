...
def test_load_cases_merges_family_defaults():
    ...
    assert ols.n_grid == list(range(20, 141, 10))          # per-case grid (capped near 80% crossing)
...
def test_load_cases_new_fields():
    cases = load_cases(ROOT / "benchmark_cases.json")
    by_id = {c.id: c for c in cases}
    assert by_id["anova_2x2"].n_grid == list(range(40, 541, 50))       # reaches the interaction's 80% crossing
    ...
    assert by_id["ols_large_n"].n_grid == list(range(300, 2101, 200))
...
def test_loop_design_factor_factor_product():
    ...
    assert spec["beta"].tolist() == [0.5, 0.5, 0.5]      # f1[2], f2[2], f1[2]:f2[2]
    assert spec["targets"].tolist() == [0]               # target "f1[2]" = first factor dummy
...
def test_loop_design_cont_factor_product():
    import numpy as np
    from loops_py import _design, _draw_X
    # standalone fixture (mirrors the retired ols_factor_inter case) — exercises
    # loops_py's continuous×factor product column without depending on the
    # benchmark case list.
    c = Case(id="cont_factor", family="ols", formula="y = x1 + f + x1:f",
             effects="x1=0.40, f[2]=0.5, x1:f[2]=0.3", targets=["x1"],
             n_grid=[100], n_sims={}, target_power=0.8,
             variable_types={"f": "(factor,0.5,0.5)"})
    spec = _design(c)                                    # cols: x1, f[2], x1*f[2]
    assert spec["beta"].tolist() == [0.40, 0.5, 0.3]
    X = _draw_X(np.random.default_rng(7), 500, spec)
...
def test_loop_design_anova_2x3_names():
    ...
    # standalone fixture (mirrors the retired anova_2x3 case) — 2x3 factorial
    # dummy/interaction naming in loops_py, independent of the benchmark list.
    c = Case(id="anova_2x3", family="ols", formula="y = f1*f2",
             effects="f1=0.5, f2[2]=0.5, f2[3]=0.5, f1:f2[2]=0.5, f1:f2[3]=0.5",
             targets=["f1"], n_grid=[100], n_sims={}, target_power=0.8,
             variable_types={"f1": "(factor,0.5,0.5)", "f2": "(factor,0.34,0.33,0.33)"})
    spec = _design(c)
