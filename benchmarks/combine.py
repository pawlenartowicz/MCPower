...
find_sample_size grid-search comparison, and (--plot / --plot-fss) two grouped
bar charts. All cross-method comparisons are per-sim normalized."""
...
def combine(py_path, r_path, py_1t_path=None, r_1t_path=None):
    ...
    The optional *_1t paths are results from a `--threads 1` harness run; only
    their mcpower_find_power rows are used, as the `mcpower-1t:<lang>` series
    (single-core engine — the same-cores comparison against the serial tools).
    """
    py_meta, py_rows = load_results(py_path)
    r_meta, r_rows = load_results(r_path)
    scale = py_meta.get("n_sims_scale") or 1.0
    if (r_meta.get("n_sims_scale") or 1.0) != scale:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Combine py + R benchmark results")
    ap.add_argument("py_json")
    ap.add_argument("r_json")
    ap.add_argument("--py-1t", default=None, metavar="JSON",
                    help="results of a --threads 1 py run (mcpower-1t series)")
    ap.add_argument("--r-1t", default=None, metavar="JSON",
                    help="results of a --threads 1 R run (mcpower-1t series)")
    ap.add_argument("--plot", default=None, metavar="PNG",
                    help="write the per-family summary bar chart")
    ap.add_argument("--plot-fss", default=None, metavar="PNG",
                    help="write the find_sample_size grid-search bar chart")
    a = ap.parse_args()
    cases = load_cases(CASES_PATH)
    py_meta, r_meta, series, tool_names, fss = combine(a.py_json, a.r_json,
                                                       a.py_1t, a.r_1t)
    print_meta(py_meta)
    print_meta(r_meta)
    print()
    ...
    if a.plot:
        write_plot((agg, coverage, SUMMARY_ORDER,
                    ...
                    "find_power: per-sim time vs dedicated tools & DIY loops"), a.plot)
        print(f"\nWrote find_power plot to {a.plot}")
    if a.plot_fss:
        write_plot((agg_fss, coverage_fss, FSS_ORDER,
                    ...
                    "every grid point"), a.plot_fss)
        print(f"\nWrote find_sample_size plot to {a.plot_fss}")
