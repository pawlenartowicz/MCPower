...
# Opt-in per-fit diagnostics for the statsmodels loop tiers. None in normal
# (timed) runs -> plain fits, no warning capture, no extra cost. diagnose_loops.py
# sets this to a dict and calls a loop *in-process* to record, per (case, n), how
# many per-sim fits raise, fail to converge, or emit which statsmodels warnings —
# so a run can tell random per-sim hiccups from a systematically unfittable
# ("impossible") case, instead of just silencing the warning.
DIAG = None


def _diag_fit(make_fit):
    """Run one per-sim statsmodels fit under the loops' skip rules; return
    (fit, usable). A fit is unusable if it raises or reports non-convergence —
    identical to the inline try/except the loops used before. When DIAG is set
    (diagnostic runs only), also capture and tally the warnings each fit raises
    and its outcome."""
    if DIAG is None:
        try:
            fit = make_fit()
            ...
            return None, False
        return fit, getattr(fit, "converged", True)
    ...
    with warnings.catch_warnings(record=True) as captured:
        ...
        try:
            ...
            return None, False
    for w in captured:
        key = f"warn:{w.category.__name__}"
        DIAG[key] = DIAG.get(key, 0) + 1
    usable = getattr(fit, "converged", True)
    outcome = "usable" if usable else "not_converged"
    DIAG[outcome] = DIAG.get(outcome, 0) + 1
    return fit, usable
...
def _lme_best_chunk(case, n, n_sims, seed):
    ...
    for _ in range(n_sims):
        ...
        Xd = sm.add_constant(X)
        # The fit itself uses the robust DEFAULT optimizer — identical to the
        # naive tier (same seed -> same data -> same fit). The "best" tier earns
        # its speed from loop-level work only: precomputed critical values, a
        # manual Wald contrast, and process parallelism. It must NOT take a fit
        # shortcut: an explicit method="lbfgs" drove the marginal covariance to a
        # rank-deficient region where statsmodels inverts a singular matrix and
        # raised LinAlgError on up to ~97% of draws, so loop_best's LME power was
        # computed from a tiny, biased survivor subsample. (See diagnose_loops.py.)
        # The default optimizer still flags ~5-20% of draws non-converged
        # ("Gradient optimization failed, |grad|=X"), but that is a statsmodels
        # artifact, not a fittability problem: its gradient-norm tolerance isn't
        # sample-size-normalized, so |grad| grows with n and trips the check on
        # fits whose fixed effects are fine (0% raise; rate non-monotonic in n,
        # 100% usable at n=400/500 — a degenerate design would fail
        # systematically). Those draws drop from the power denominator below;
        # the fit still ran, so timing is unaffected and the dropped-draw
        # selection bias touches only the sanity-signal power. Left as-is
        # deliberately; quantified by diagnose_loops.py.
        fit, usable = _diag_fit(lambda: sm.MixedLM(y, Xd, groups=cluster_ids).fit(disp=False))
        if not usable:
            continue
        converged += 1
...
def lme_naive(case, n, n_sims, seed):
    ...
    for _ in range(n_sims):
        ...
        y = X @ BETA + b_cluster[cluster_ids] + eps
        Xd = sm.add_constant(X)
        # Same MixedLM fit as lme_best; see its comment for the benign ~5-20%
        # non-convergence (statsmodels gradient-norm artifact) left as-is.
        fit, usable = _diag_fit(lambda: sm.MixedLM(y, Xd, groups=cluster_ids).fit(disp=False))
        if not usable:
            continue
        converged += 1
