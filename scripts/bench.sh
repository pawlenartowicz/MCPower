...
#   scripts/bench.sh --scenarios off
...
# Both underlying tools refuse --save with --case/--scenarios (partial baseline).
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --scenarios)
      ...
      [[ "$MODE" == "off" || "$MODE" == "on" ]] || { echo "bench.sh: --scenarios requires off|on" >&2; exit 1; }
      ARGS+=("--scenarios" "$MODE"); shift 2 ;;
    --case)
      ARGS+=("--case" "${2:?--case requires a case id}"); shift 2 ;;
    --save) ARGS+=("--save"); shift ;;
    --force) FORCE=1; shift ;;
    *) echo "bench.sh: unknown flag $1; usage: bench.sh [--case <id>] [--scenarios off|on] [--save] [--force]" >&2; exit 1 ;;
  esac
done
