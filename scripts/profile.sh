...
# case ids (and --scenarios filter) as the throughput bench, applies the two-build
...
#   scripts/profile.sh glm_rare ols_large_n --scenarios on
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --scenarios)
      MODE="${2:?--scenarios requires off|on}"
      [[ "$MODE" == "off" || "$MODE" == "on" ]] || { echo "profile.sh: --scenarios requires off|on" >&2; exit 1; }
      shift 2 ;;
    --force) FORCE=1; shift ;;
...
for CASE in "${CASES[@]}"; do
  ...
  TIMING="$(taskset -c "$PIN_CORE" "$RUN_SCRATCH/throughput-nodebug" --case "$CASE" --scenarios "$MODE")"
  echo "$TIMING"
  ...
  for BUILD in nodebug lines; do
    perf record -F "$PERF_FREQ" -o "$RUN_SCRATCH/$CASE-$MODE-$BUILD.data" -- \
      taskset -c "$PIN_CORE" "$RUN_SCRATCH/throughput-$BUILD" --case "$CASE" --scenarios "$MODE" \
      > /dev/null
  done
