...
import type { EffectSize, CsvData, AppSpec } from '../src/types';
...
describe('getEffectsFromData', () => {
  ...
  it('calls wasm get_effects_from_data with the JSON-serialized spec and returns parsed EffectSize[]', async () => {
    const mockEffects: EffectSize[] = [{ name: 'x', value: 0.42 }];
    vi.mocked(wasmMod.get_effects_from_data).mockReturnValue(JSON.stringify(mockEffects));
