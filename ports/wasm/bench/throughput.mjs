...
//   taskset -c 2 node bench/throughput.mjs --scenarios off
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--save') args.save = true;
      ...
      args.mode = argv[++i];
      if (args.mode !== 'off' && args.mode !== 'on') die('--scenarios requires off or on');
    } else if (a === '--artifact') args.artifact = argv[++i] ?? die('--artifact requires a dir');
    else die(`unknown flag ${a}; usage: throughput.mjs [--save] [--case <id>] [--scenarios off|on] [--force] [--artifact <dir>]`);
  }
  if (args.save && (args.case || args.mode)) {
    die('--save with --case/--scenarios would overwrite the baseline with a partial run; drop one');
  }
  return args;
}
