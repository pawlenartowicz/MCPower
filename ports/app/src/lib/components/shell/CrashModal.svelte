<script lang="ts">
  ...
  import { openExternal, reportUrl } from '$lib/content/render-doc';
  ...
  // Open the report form pre-filled with the crash dump (title + message + detail).
  function report() {
    const dump = [crash?.title, crash?.message, crash?.detail].filter(Boolean).join('\n\n');
    openExternal(reportUrl(dump));
  }
</script>
...
<Dialog open={crash != null} onOpenChange={(v) => { if (!v) close(); }}>
  <DialogContent showCloseButton={false}>
    ...
    <DialogFooter>
      ...
      {/if}
      <Button variant="outline" onclick={report}>Report this</Button>
