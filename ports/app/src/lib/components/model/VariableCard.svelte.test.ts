...
describe('VariableCard factor locked', () => {
  it('shows the kind badge but no "from data" badge when locked', () => {
    const v = factorRow();
    ...
    // The detected kind is still shown; the redundant "from data" badge was removed.
...
describe('VariableCard factor locked — effects stay editable', () => {
  it('factor effect input (non-reference level) is NOT disabled when locked', () => {
    ...
    const effects = [
      { name: 'cyl[6]', value: 0.3 },
      { name: 'cyl[8]', value: 0.5 },
    ];
    const { container } = render(VariableCard, {
  ...
  it('binary effect input is NOT disabled when locked', () => {
    ...
    const effectInput = container.querySelector(
      '[data-testid="effect-treated"] input[type="number"]',
    );
  ...
  it('type Select reappears and nLevels input enabled after locked transitions to false', async () => {
    ...
    const { container, rerender } = render(VariableCard, {
      props: {
        ...
        group: g,
        effects: [
          { name: 'cyl[6]', value: 0.25 },
          { name: 'cyl[8]', value: 0.25 },
        ],
        variables: [v],
        locked: true,
      },
...
describe('VariableCard interactionOnly', () => {
  it('shows the :/* note and no effect number input for an interaction-only var', () => {
    const v = continuousRow();
    const group: VariableGroup = {
      name: v.name,
      kind: 'continuous',
    ...
    const { container, getByTestId } = render(VariableCard, {
      props: {
        variable: v,
        group,
        effects: [],
    ...
    // The :/* explanation note renders...
    const note = getByTestId(`interaction-only-note-${v.name}`);
    expect(note.textContent).toContain('main effect');
    // ...and no effect-size input is offered. A continuous var has no header
    // number inputs (no levels/share), so any number input would be the β
    // stepper from EffectControls — there must be none.
    expect(container.querySelector('input[type="number"]')).toBeNull();
    expect(container.querySelector(`[data-testid="effect-${v.name}"]`)).toBeNull();
  });

  it('a normal predictor still renders its effect control (no note)', () => {
    const v = continuousRow();
    const { container, queryByTestId } = render(VariableCard, {
      props: {
        variable: v,
        group: continuousGroup(v),
        effects: [{ name: v.name, value: 0 }],
        variables: [v],
      },
    });
    expect(queryByTestId(`interaction-only-note-${v.name}`)).toBeNull();
