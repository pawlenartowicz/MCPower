...
async function startFindPowerFromShortcut() {
  // Mirror StatusBar's Run button: drive the active family (regression/anova/
  // mixed), not just regression. Non-wired families take the demo path rather
  // than silently no-opping.
  if (!SELECTABLE_FAMILIES.includes(familyStore.active)) { startRun('find-power'); return; }
  const cfg = familyStore.byFamily[familyStore.active];
  const { spec, errors } = familyConfigToAppSpec(
    familyStore.active, cfg, familyStore.regressionOutcome,
  );
...
async function startFindSampleSizeFromShortcut() {
  if (!SELECTABLE_FAMILIES.includes(familyStore.active)) { startRun('find-sample-size'); return; }
  const cfg = familyStore.byFamily[familyStore.active];
  const { spec, errors } = familyConfigToAppSpec(
    familyStore.active, cfg, familyStore.regressionOutcome,
  );
