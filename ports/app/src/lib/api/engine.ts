...
import type { AppSpec, EffectDescriptor, EffectSize } from '$lib/domain/app-spec';
export type { EffectDescriptor, EffectSize };
import type {
  PowerResult,
  SampleSizeResult,
  SampleSizeMethod,
...
/**
 * Recover standardized effect sizes from the uploaded data in `spec.csv`.
 * Returns an empty array when `spec.csv` is missing or malformed (no CSV column data).
 * The fitted estimator follows the spec family (OLS/GLM/MLE).
 */
export async function getEffectsFromData(spec: AppSpec): Promise<EffectSize[]> {
  const { invoke } = await api();
  return invoke<EffectSize[]>('get_effects_from_data_cmd', { spec });
}
