...
describe('routeMenuEvent — run shortcuts', () => {
  ...
  it('run.find_power is a no-op when active family is not regression', () => {
    familyStore.active = 'anova' as any;
    const spy = vi.spyOn(runStore, 'startFindPower');
    routeMenuEvent('run.find_power');
    // async helper returns early — startFindPower should not be called
    expect(spy).not.toHaveBeenCalled();
    spy.mockRestore();
    familyStore.active = 'regression';
  });

  it('run.find_n is a no-op when active family is not regression', () => {
    familyStore.active = 'mixed' as any;
    const spy = vi.spyOn(runStore, 'startFindSampleSize');
