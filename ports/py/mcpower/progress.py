...
explicit ``False`` cancels the run. If a user callable *raises*, the engine
treats it as a cancel request: the exception is caught at the Rust boundary
(``engine-py`` ``orchestrator_bridge``) and the run stops cleanly — the original
exception is not re-raised in the caller.
"""
