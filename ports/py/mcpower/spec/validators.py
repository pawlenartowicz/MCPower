...
def _validate_upload_data(data: np.ndarray, columns: Optional[List[str]] = None) -> _ValidationResult:
    ...
    # NaN/Inf check on numeric columns. Object dtype (string columns) cannot be cast
    # to float, so we check column-by-column and skip non-numeric ones.
    nan_cols: List[str] = []
    inf_cols: List[str] = []
