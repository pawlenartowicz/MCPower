...
class VariableRegistry:
    ...
    def get_effect_sizes(self) -> List[float]:
        """Get effect sizes as a list of floats in effect order."""
        return [float(eff.effect_size) for eff in self._effects.values()]

    def get_correlation_matrix(self) -> Optional[List[List[float]]]:
        """Get correlation matrix for non-factor variables (list-of-lists)."""
        return self._correlation_matrix

    def set_correlation_matrix(self, matrix: List[List[float]]) -> None:
        """Set correlation matrix for non-factor variables (deep-copied)."""
        self._correlation_matrix = [list(row) for row in matrix]
