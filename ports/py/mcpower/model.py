  ...
  Two independent axes control every analysis:
    ...
    * ``estimator=`` / ``solve_as=`` — the statistical model fitted to each
    ...
    def correlation_matrix(self) -> Optional[np.ndarray]:
        """Correlation matrix for non-factor predictors (read-only)."""
    ...
    def set_effects(self, spec: Union[str, Dict[str, float]]) -> "MCPower":
        ...
        if isinstance(spec, dict):
            self._pending_effects = ", ".join(f"{k}={v}" for k, v in spec.items())
        elif isinstance(spec, str):
            if not spec.strip():
                raise ValueError("effects spec cannot be empty")
            self._pending_effects = spec
        else:
    ...
    def set_correlations(
        self, spec: Union[str, np.ndarray, Dict[Tuple[str, str], float]]
    ) -> "MCPower":
        """Set correlations between non-factor predictors.

        Accepts a v1-style assignment string (``"corr(x1,x2)=0.3"``), a
        full numpy correlation matrix, or a dict keyed by ``(var1, var2)``
        tuples.
        """
        if isinstance(spec, dict):
            # Render dict back into the assignment-string form the parser uses.
            self._pending_correlations = ", ".join(
                f"corr({a},{b})={v}" for (a, b), v in spec.items()
            )
        elif isinstance(spec, (str, np.ndarray)):
            self._pending_correlations = spec
        else:
            raise TypeError("set_correlations expects a string, ndarray, or dict")
        self._applied = False
        return self
    ...
    def set_variable_type(self, spec: str) -> "MCPower":
        ...
        self._pending_variable_types = spec
        self._applied = False
        return self
    ...
    def upload_data(
        ...
        arr, cols = normalize_upload_input(data, columns)
        _validate_upload_data(arr, cols).raise_if_invalid()
        ...
        if verbose:
            print(f"Uploaded {arr.shape[0]} rows, {len(cols)} columns.")
            for i, col_name in enumerate(cols):
        ...
        self._pending_data = {
            ...
            "mode": mode,
            "uploaded_n": arr.shape[0],
        }
        self._uploaded_data_n = arr.shape[0]
    ...
    def get_effects_from_data(self, y: str, *, verbose: bool = True) -> str:
        ...
        the native outcome (raw 0/1 / raw response). MLE recovery is
        ...
        # MLE (mixed) recovery is fixed-effects-only: the uploaded data must
        # carry the grouping column. Map its distinct values to contiguous
        # 0-based cluster IDs and pin the contract's cluster count to the data's
        # distinct-group count so the LME fitter's per-cluster buffers line up.
    ...
    def _apply(self) -> None:
        ...
        if self._pending_effects:
            ...
            assignments = _engine.split_assignments(self._pending_effects)
        ...
        # 5. Correlations.
        if self._pending_correlations is not None:
            non_factor = self._registry.non_factor_names
            n = len(non_factor)
            if self._registry.get_correlation_matrix() is None:
                self._registry.set_correlation_matrix(np.eye(n))
            corr_input = self._pending_correlations
            ...
            elif isinstance(corr_input, np.ndarray):
                if corr_input.shape != (n, n):
                    raise ValueError(
                        f"Matrix shape {corr_input.shape} doesn't match {n} "
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effects": dict(zip(
                ...
                [float(b) for b in self._registry.get_effect_sizes().tolist()],
                strict=False,
            )),
