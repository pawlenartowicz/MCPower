...
class Report:
    ...
    def _ci_section(self, cfg) -> str:
        """§5.1 — one Power & 95% CI table per scenario (find_power only).
    ...
    def _required_n_ci_table(self, cfg) -> str:
        """§5.2 — Required N & 95% CI table from the model-based crossing fit.
    ...
    def _estimator_extras(self, cfg) -> str:
        """§5.4 — surface GLM/MLE numerics whenever present (not only on a
        threshold trip). OLS carries only {'estimator': ...} -> nothing shown.
        One block per scenario (mirroring §5.1) so a doomer scenario hitting
