...
def add_boundary_hit_rates(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``boundary_hit`` is a numpy uint8 array of shape ``(n_sims, n_sample_sizes)``.
    ...
    always zero so both lists will contain only 0.0 values.

    The function mutates *and* returns the dict so callers can use it inline.
    """
    if "scenarios" in result:
        # Multi-scenario envelope: recurse into each scenario dict.
    ...
    if bh is None:
        # Fallback: no boundary_hit key at all — add zero-filled placeholders.
    ...
    bh_arr = np.asarray(bh, dtype=np.uint8)
    # Shape: (n_sims, n_sample_sizes).  n_sims may be 0 on an empty run.
    if bh_arr.ndim == 1:
        bh_arr = bh_arr[:, np.newaxis]
    n_sims, n_ss = bh_arr.shape
    if n_sims == 0:
        result["boundary_hit_rate_tau_zero"] = [0.0] * n_ss
        result["boundary_hit_rate_high_tau"] = [0.0] * n_ss
        return result
    ...
    result["boundary_hit_rate_tau_zero"] = [
        float(np.mean(bh_arr[:, j] == 1)) for j in range(n_ss)
    ]
    result["boundary_hit_rate_high_tau"] = [
        float(np.mean(bh_arr[:, j] == 2)) for j in range(n_ss)
    ]
    return result
