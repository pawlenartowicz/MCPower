...
def test_e5_boundary_hit_ols_find_power():
    """Each scenario payload must carry 'boundary_hit' as a uint8 numpy array.
    ...
    assert hasattr(bh, "dtype"), "boundary_hit is not a numpy array"
    assert bh.dtype == np.uint8, f"expected dtype uint8, got {bh.dtype}"
    assert bh.ndim == 2, f"expected 2-D array, got shape {bh.shape}"
    assert bh.shape == (n_sims, 1), f"expected shape ({n_sims}, 1), got {bh.shape}"
    assert np.all(bh == 0), "OLS boundary_hit must be all zeros"
