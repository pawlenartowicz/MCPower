...
def test_summary_renders_when_leading_term_dropped() -> None:
    ...
    # corr(study, caffeine)=0.6 induces OVB when fitting only caffeine → power
    # well above α but not at 1.0. Seeded value at seed=2137, N=100: 0.435.
    assert result["n_targets"] == 1
    assert 0.20 < result["power_uncorrected"][0][0] < 0.80
