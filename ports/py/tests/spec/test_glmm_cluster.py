...
def test_glmm_scenario_re_knob_reaches_engine():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        result = m.find_power(100, n_sims=20, verbose=False)
    assert "power_uncorrected" in result
    assert 0.0 <= result["power_uncorrected"][0][0] <= 1.0


# ---------------------------------------------------------------------------
# Task 1.4 — End-to-end GLMM smoke + §6.2-M4 port audit
# ---------------------------------------------------------------------------


def test_glmm_end_to_end_find_power():
    """§6.2-M4 audit: family='logit' + (1|group) + set_cluster reaches the GLMM
