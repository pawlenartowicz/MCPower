...
def test_validate_upload_data_rejects_bad_arrays():  # APIC-16
    # Column-major input: outer list = columns, inner list = the column's rows.
    # (The old ndim!=2 guard is gone — normalize_upload_input now structurally
    #  guarantees a list-of-columns, so a 1-D case can't reach the validator.)
    assert not _validate_upload_data([[1.0] * 19, [1.0] * 19]).is_valid   # < min_rows (20)
    assert _validate_upload_data([[1.0] * 20, [1.0] * 20]).is_valid       # == min_rows (20) → ok
    assert not _validate_upload_data([[1.0] * 1_000_001]).is_valid        # > max_rows (1M, native cap)
    nan_cols = [[1.0] * 30, [1.0] * 30]
    nan_cols[0][0] = float("nan")
    assert not _validate_upload_data(nan_cols).is_valid                   # NaN
    inf_cols = [[1.0] * 30, [1.0] * 30]
    inf_cols[1][1] = float("inf")
    assert not _validate_upload_data(inf_cols).is_valid                   # Inf
    assert _validate_upload_data([[1.0] * 30, [1.0] * 30]).is_valid       # clean
