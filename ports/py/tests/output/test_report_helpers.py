...
def test_fmt_target_has_no_glyph():
    ...
    # ✓ / ✗ are gone from the output character set (charter §1)
...
def test_main_power_tables_scenarios_only_one_table_no_delta():
    ...
    assert "Δ" not in tables[0] and "pp" not in tables[0]   # drop moved to §5.3
...
def test_fmt_ci_includes_percent_and_aligns_100():
    ...
    # space so it stacks under "99.0%" (charter §5.1, cross-port parity).
