"""Power for a binary outcome with family='logit'; set_baseline_probability fixes the event rate at the reference level, effects are log-odds shifts per SD (or per category for binary predictors)."""
...
model = MCPower("response = dose + age + sex", family="logit")
...
print(">>> model.find_power(sample_size=300, target_test='all')")
