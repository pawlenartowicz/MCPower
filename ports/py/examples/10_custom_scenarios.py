"""Tuning the scenario sweep; set_scenario_configs overrides the built-in optimistic/realistic/doomer presets and defines custom assumption sets that find_power then runs side by side."""
...
model = MCPower("outcome = treatment + experience")
...
print(">>> model.find_power(sample_size=150, target_test='all', scenarios=True)")
model.find_power(sample_size=150, target_test="all", scenarios=True)
...
#    list to scenarios= to control exactly which appear, and in what order.
#    Long form via .summary() adds the "Robustness" table — each scenario's
#    Δ power against the optimistic baseline — and the joint distribution.
print("\n>>> result = model.find_power(sample_size=150, target_test='all',")
print("...                            scenarios=['optimistic', 'realistic', 'stress_test'], verbose=False)")
print(">>> print(result.summary())")
result = model.find_power(
    sample_size=150,
    target_test="all",
    scenarios=["optimistic", "realistic", "stress_test"],
    verbose=False,
)
