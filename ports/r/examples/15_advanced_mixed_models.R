# Advanced Mixed-Model Designs
# ============================
#
# Four advanced mixed-model designs, one step beyond the basic random intercept
# of example 08: random slopes, nested groupings, a clustered logistic GLMM
# whose treatment is assigned at the cluster level, and random-effect scenario
# stress-testing. Each section is self-contained — build a model, ask for power.
#
...
model <- MCPower$new("rt ~ dose + (1 + dose|subject)", family = "lme")
model$set_effects("dose=0.3")

# random_slopes is a list of per-slope specs: predictor names the predictor
# whose slope varies, variance is the spread of those per-subject slopes, and
# corr_with_intercept their correlation with the random intercept. 30 subjects,
# ICC=0.20 between-subject baseline.
model$set_cluster("subject", ICC = 0.20, n_clusters = 30,
                  random_slopes = list(list(predictor = "dose", variance = 0.15,
                                            corr_with_intercept = 0.0)))

cat(">>> model$find_power(sample_size = 300, target_test = 'dose')\n")
invisible(model$find_power(sample_size = 300, target_test = "dose"))
...
model <- MCPower$new("score ~ teaching_method + (1|school/classroom)", family = "lme")
model$set_variable_type("teaching_method=binary")
model$set_effects("teaching_method=0.4")

# 10 schools x 3 classrooms each = 30 classrooms total. n_per_parent=3 is what
...
cat(">>> model$find_power(sample_size = 300, target_test = 'teaching_method')\n")
invisible(model$find_power(sample_size = 300, target_test = "teaching_method"))
...
cat(">>> model$find_power(sample_size = 600, target_test = 'treatment, baseline_severity')\n")
invisible(model$find_power(sample_size = 600, target_test = "treatment, baseline_severity"))
...
model <- MCPower$new("score ~ teaching_method + prior_gpa + (1|classroom)", family = "lme")
model$set_variable_type("teaching_method=binary")
...
model$set_scenario_configs(list(
  re_stress = list(random_effect_dist = "heavy_tailed",
...
# uncertain ICC.
cat(">>> result <- model$find_power(sample_size = 300, target_test = 'teaching_method',\n")
cat("...   scenarios = c('optimistic', 're_stress'))\n")
invisible(model$find_power(sample_size = 300, target_test = "teaching_method",
                           scenarios = c("optimistic", "re_stress")))
