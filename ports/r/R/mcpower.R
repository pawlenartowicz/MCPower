...
MCPower <- R6::R6Class(
  ...
  public = list(
    ...
    seed = NULL,             # base RNG seed (default 2137; NULL = random)
    ...
    # ------------------------------------------------------------------
    # Configuration setters (model.py:414-549). Each returns invisible(self).
    # ------------------------------------------------------------------
    ...
    # Compatibility alias for v1's pluralized name (model.py:490).
    ...
    set_variable_type = function(spec) {
      ...
      private$pending_variable_types <- spec
      private$applied <- FALSE
      invisible(self)
    },
    ...
    set_cluster = function(grouping_var, ICC = NULL, n_clusters = NULL,
      ...
      # Store cluster-level covariate names separately; emitted into LinearSpec
      # payload as `cluster_level_vars` (engine GenerationSpec.cluster_level_columns).
      private$pending_cluster_level_vars <- cluster_level_vars
      private$applied <- FALSE
      invisible(self)
    },
    ...
    # model.py:777-867 upload_data — ingest empirical predictor data.
    ...
    # model.py:869-1076 get_effects_from_data — estimate standardized effects from
    # uploaded data. Estimator follows the family: OLS (linear) z-scores the
    # outcome; GLM (logit) and MLE (mixed) fit the native outcome (MLE is
    # fixed-effects-only and needs the grouping column uploaded). Returns a
    # set_effects-style string "x=0.13, y=0.41"; does NOT auto-apply the effects.
    get_effects_from_data = function(y, verbose = TRUE) {
      ...
      # MLE (mixed) recovery is fixed-effects-only: thread the uploaded grouping
      # column as contiguous 0-based cluster IDs and pin the contract's cluster
      # count to the data's distinct-group count so the LME fitter's per-cluster
      # buffers line up.
      cluster_ids <- integer(0)
      if (identical(self$estimator, "mle")) {
        grouping_var <- names(private$pending_clusters)[1]
        if (is.null(grouping_var) || !grouping_var %in% names(raw_columns)) {
          stop(paste0(
            "mixed-model get_effects_from_data needs the grouping column ",
    ...
    # model.py:295-299 set_parallel tombstone — parallelism is automatic.
    ...
    # ------------------------------------------------------------------
    # Analysis entry points (model.py:1622-1843).
    # ------------------------------------------------------------------
    ...
    find_power = function(sample_size, target_test = NULL, correction = NULL,
      ...
      # Logit pre-flight (model.py:853-897).
      ...
      # exceed the configured threshold. Mirrors Python model.py:1482-1484.
      ...
      # Strict-bootstrap reuse diagnostics (mirrors model.py:1987-2000).
    ...
    find_sample_size = function(target_test = NULL, correction = NULL,
      ...
      # LME cluster_size-only guard (model.py:998-1006).
      ...
      raw <- find_sample_size(
        ...
        progress)
      raw <- .unwrap_scenario_result(raw, names)
      # Post-batch: check failure threshold (mirrors Python model.py:1680).
      .check_result_failure_threshold(raw, self$max_failed_simulations)
      ...
      .check_glmm_laplace_bias_warning(raw, cluster_size = min_cs)
      if (isTRUE(verbose)) print(result)
      # Strict-bootstrap reuse diagnostics per achieved-N (mirrors model.py:2189-2212).
      if (!is.null(private$pending_data) &&
          identical(private$pending_data$mode, "strict") &&
          private$pending_data$uploaded_n > 0L) {
        U     <- as.integer(private$pending_data$uploaded_n)
  ...
  active = list(
    # model.py:276-279 equation property.
  ...
  private = list(
    ...
    pending_cluster_level_vars = NULL,
    ...
    # model.py:1116-1209 _apply — apply pending settings to the registry.
    apply = function() {
      reg <- private$registry

      # 1. Variable types.
      if (!is.null(private$pending_variable_types)) {
        reg$set_variable_types(private$pending_variable_types)
      }
      ...
      # 3. Effects.
      if (!is.null(private$pending_effects)) {
        reg$set_effects(private$pending_effects)
      }

      # 4. Baseline probability -> intercept (logit only; no-op otherwise).
      private$apply_baseline_probability()

      # 5. Correlations — a matrix takes the full-matrix path, a string the
      #    assignment-spec path (model.py:1383-1416).
      if (!is.null(private$pending_correlations)) {
        if (is.matrix(private$pending_correlations)) {
          private$apply_correlation_matrix(private$pending_correlations)
        } else {
          reg$set_correlation_spec(private$pending_correlations)
        }
      }
    ...
    # model.py:1407-1416 — full-matrix correlation branch of _apply().
    ...
    # model.py:838-851 _apply_baseline_probability.
    ...
    # model.py:551-568 _validate_correction_arg — validate correction name.
    ...
    # model.py:853-897 _validate_logit_runtime.
    ...
    # cluster_size-only LME guard for find_sample_size (model.py:998-1006).
    ...
    # model.py:1586-1620 _resolve_scenarios_arg.
    ...
    build_contract_bytes = function(scenario_names, target_test = NULL,
      ...
      payload <- .to_linear_spec_list(
        ...
        cluster_level_vars = private$pending_cluster_level_vars,
        estimator = self$estimator)
