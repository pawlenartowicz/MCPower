...
# Mirror of parsers.py:107-114 _terms_to_fixed_string.
...
# Mirror of parsers.py:117-134 _re_to_legacy_dict.
...
# Mirror of parsers.py:33-55 _parse_equation. Returns list(dependent,
...
# Mirror of parsers.py:58-104 _parse_independent_variables. Returns
...
# ---------------------------------------------------------------------------
# variables.py:67-91 _resolve_reference — resolve a by-VALUE factor reference
...
RVariableRegistry <- R6::R6Class(
  ...
  active = list(
    ...
    # variables.py:153-155 dependent
    dependent = function() private$dependent_,

    # variables.py:158-160 predictor_names — sorted by column_index.
    ...
    # variables.py:182-184 cluster_effect_names
    ...
    # variables.py:187-189 dummy_names
  ...
  public = list(
    ...
    # variables.py:212-221 factor_levels — full ordered label list for factor
    ...
    # variables.py:191-193 get_predictor
    get_predictor = function(name) private$predictors[[name]],

    # variables.py:195-197 get_effect
    get_effect = function(name) private$effects[[name]],

    # variables.py:199-232 set_variable_type
    ...
    # variables.py:234-245 set_effect_size
    ...
    # variables.py:247-367 expand_factors
    ...
    # model.py:1119-1131 — variable types branch of _apply().
    ...
    # model.py:1138-1158 — effects branch of _apply() (validation + set).
    ...
    # model.py:1175-1199 — correlation branch of _apply() (string path).
  ...
  private = list(
    ...
    # variables.py:369-378 _update_effect_indices.
...
# ---------------------------------------------------------------------------
# model.py mirror — _scenario_dict (nested inner fn, model.py:1215-1273)
# ---------------------------------------------------------------------------
...
# Mirror of model.py:1215-1273 _scenario_dict. `scenario_configs` defaults to
...
# ---------------------------------------------------------------------------
# model.py mirror — _resolve_tests (model.py:570-770)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# model.py mirror — _to_linear_spec_dict (model.py:1275-1464)
# ---------------------------------------------------------------------------
...
.to_linear_spec_list <- function(reg, scenario_names, alpha, correction,
                                  ...
                                  estimator = "ols") {
  # Predictors — non-factors first (formula order), then factors. (model.py:1305-1353)
  predictors <- list()
  ...
  # Effects — skip cluster effects (random, not tested). (model.py:1357-1365)
  ...
  # Correlations — non-zero off-diagonal only. (model.py:1370-1384)
  correlations <- list()
  ...
  # Correction (model.py:1390-1393).
  correction_wire <- .correction_for_rust(correction)

  # Scenarios (model.py:1398).
  ...
  # Formula — strip `(1|group)` when random effects present. (model.py:1405-1410)
  ...
  # targets / contrast_pairs / report_overall. (model.py:1417-1441)
  ...
  # Upload block: emit when pending_data is set. (model.py:1674-1705)
...
# ---------------------------------------------------------------------------
# model.py mirror — _encode_outcome_and_clusters (model.py:1512-1543) +
# _build_cluster_spec_dict (model.py:1545-1580)
# ---------------------------------------------------------------------------
