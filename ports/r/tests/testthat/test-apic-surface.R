...
test_that("set_scenario_configs accepts mixed-model knobs (Pass B wiring)", {
  ...
  for (key in c("icc_noise_sd", "random_effect_dist", "random_effect_df")) {
    ...
    expect_no_error(m$set_scenario_configs(cfg))
  }
})
...
test_that("set_residual_distribution accepts all five canonical names; aliases removed (APIC-73)", {
  # All five canonical names are accepted (no alias layer anymore).
  expect_no_error(MCPower$new("y ~ x1")$set_residual_distribution("normal"))
  expect_no_error(MCPower$new("y ~ x1")$set_residual_distribution("right_skewed"))
  expect_no_error(MCPower$new("y ~ x1")$set_residual_distribution("left_skewed"))
  expect_no_error(MCPower$new("y ~ x1")$set_residual_distribution("high_kurtosis"))
